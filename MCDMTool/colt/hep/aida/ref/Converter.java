...
public class Converter {
	...
	public String toString(IHistogram1D h) {
		...
		cern.colt.matrix.impl.Former f = new cern.colt.matrix.impl.FormerFactory()
				.create(format);
		String sep = System.getProperty("line.separator");
		int[] minMaxBins = h.minMaxBins();
		String title = h.title() + ":" + sep + "   Entries="
				+ form(f, h.entries()) + ", ExtraEntries="
				+ form(f, h.extraEntries()) + sep + "   Mean="
				+ form(f, h.mean()) + ", Rms=" + form(f, h.rms()) + sep
				+ "   MinBinHeight=" + form(f, h.binHeight(minMaxBins[0]))
				+ ", MaxBinHeight=" + form(f, h.binHeight(minMaxBins[1])) + sep
				+ "   Axis: " + "Bins=" + form(f, h.xAxis().bins()) + ", Min="
				+ form(f, h.xAxis().lowerEdge()) + ", Max="
				+ form(f, h.xAxis().upperEdge());
		...
		cern.colt.matrix.DoubleMatrix2D heights = new cern.colt.matrix.impl.DenseDoubleMatrix2D(
				1, h.xAxis().bins());
		...
		return title
				...
				+ new cern.colt.matrix.doublealgo.Formatter().toTitleString(
						heights, yEdges, xEdges, rowAxisName, columnAxisName,
						null, aggr);
		/*
		 * + sep + "Errors:" + sep + new
	...
	public String toString(IHistogram2D h) {
		...
		String title = h.title() + ":" + sep + "   Entries="
				+ form(f, h.entries()) + ", ExtraEntries="
				+ form(f, h.extraEntries()) + sep + "   MeanX="
				+ form(f, h.meanX()) + ", RmsX=" + form(f, h.rmsX()) + sep
				+ "   MeanY=" + form(f, h.meanY()) + ", RmsY="
				+ form(f, h.rmsX()) + sep + "   MinBinHeight="
				+ form(f, h.binHeight(minMaxBins[0], minMaxBins[1]))
				+ ", MaxBinHeight="
				+ form(f, h.binHeight(minMaxBins[2], minMaxBins[3])) + sep +

				"   xAxis: " + "Bins=" + form(f, h.xAxis().bins()) + ", Min="
				+ form(f, h.xAxis().lowerEdge()) + ", Max="
				+ form(f, h.xAxis().upperEdge()) + sep +

				"   yAxis: " + "Bins=" + form(f, h.yAxis().bins()) + ", Min="
				+ form(f, h.yAxis().lowerEdge()) + ", Max="
				+ form(f, h.yAxis().upperEdge());
	...
	public String toString(IHistogram3D h) {
		...
		String title = h.title()
				...
				+ form(f, h.binHeight(minMaxBins[0], minMaxBins[1],
						minMaxBins[2]))
				+ ", MaxBinHeight="
				+ form(f, h.binHeight(minMaxBins[3], minMaxBins[4],
						minMaxBins[5])) + sep +
				...
				"   yAxis: " + "Bins=" + form(f, h.yAxis().bins()) + ", Min="
				+ form(f, h.yAxis().lowerEdge()) + ", Max="
				+ form(f, h.yAxis().upperEdge()) + sep +

				"   zAxis: " + "Bins=" + form(f, h.zAxis().bins()) + ", Min="
				+ form(f, h.zAxis().lowerEdge()) + ", Max="
				+ form(f, h.zAxis().upperEdge());
		...
		cern.colt.matrix.DoubleMatrix3D heights = new cern.colt.matrix.impl.DenseDoubleMatrix3D(
				toArrayHeights(h));
		...
		return title
				...
				+ sep
				+ new cern.colt.matrix.doublealgo.Formatter().toTitleString(
						heights, zEdges, yEdges, xEdges, sliceAxisName,
						rowAxisName, columnAxisName, "", aggr);
		/*
		 * + sep + "Errors:" + sep + new
		 * cern.colt.matrix.doublealgo.Formatter().toTitleString(
		 * errors,yEdges,xEdges,rowAxisName,columnAxisName,null,aggr);
	...
	public String toXML(IHistogram1D h) {
		...
		buf.append("<statistic name=\"Entries\" value=\"" + h.entries()
				+ "\"/>");
		buf.append(sep);
		buf.append("<statistic name=\"Underflow\" value=\""
				+ h.binEntries(h.UNDERFLOW) + "\"/>");
		buf.append(sep);
		buf.append("<statistic name=\"Overflow\" value=\""
				+ h.binEntries(h.OVERFLOW) + "\"/>");
		buf.append(sep);
	...
	public String toXML(IHistogram2D h) {
		...
		out.append(sep);
		out.append("<bins2d title=\"" + h.title() + "\" xSize=\""
				+ h.xAxis().bins() + "\" ySize=\"" + h.yAxis().bins() + "\">");
		out.append(sep);
		for (int i = 0; i < h.xAxis().bins(); i++)
		...
		// out.append("<statistic name=\"Entries\" value=\""+h.entries()+"\"/>");
		// out.append(sep);
