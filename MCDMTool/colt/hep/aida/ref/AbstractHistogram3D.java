...
abstract class AbstractHistogram3D extends Histogram implements IHistogram3D {
	 ...
	 */
	protected abstract IHistogram2D internalSliceXY(String title, int indexZ1,
			int indexZ2);
	 ...
	 * flow bins <b>Note 3</b>There is no note 3 (yet)
	 */
	protected abstract IHistogram2D internalSliceXZ(String title, int indexY1,
			int indexY2);
	 ...
	 * flow bins <b>Note 3</b>There is no note 3 (yet)
	 */
	protected abstract IHistogram2D internalSliceYZ(String title, int indexX1,
			int indexX2);
	...
	public IHistogram2D projectionXY() {
		...
		return internalSliceXY(newTitle, mapZ(IHistogram.UNDERFLOW),
				mapZ(IHistogram.OVERFLOW));
	}

	public IHistogram2D projectionXZ() {
		String newTitle = title() + " (projectionXZ)";
		return internalSliceXZ(newTitle, mapY(IHistogram.UNDERFLOW),
				mapY(IHistogram.OVERFLOW));
	}

	public IHistogram2D projectionYZ() {
		String newTitle = title() + " (projectionYZ)";
		return internalSliceYZ(newTitle, mapX(IHistogram.UNDERFLOW),
				mapX(IHistogram.OVERFLOW));
	}
