...
public class CharArrayList extends AbstractCharList {
	 ...
	 */
	public void replaceFromToWithFrom(int from, int to, AbstractCharList other,
			int otherFrom) {
		// overridden for performance only.
		if (!(other instanceof CharArrayList)) {
			// slower
		...
		if (length > 0) {
			...
			System.arraycopy(((CharArrayList) other).elements, otherFrom,
	...
	public void sortFromTo(int from, int to) {
		 ...
		 */
		final int widthThreshold = 10000; // never consider options resulting in
