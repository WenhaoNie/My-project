...
public class ObjectArrayList extends AbstractList {
	 ...
	 *                <tt>index &lt; 0 || index &gt; size()</tt>).
	 */
	public void beforeInsertAllOfFromTo(int index, ObjectArrayList other,
			int from, int to) {
	...
	public int binarySearchFromTo(Object key, int from, int to,
		...
		return cern.colt.Sorting.binarySearchFromTo(this.elements, key, from,
				to, comparator);
	}
		...
		if (!testForEquality) {
			...
			for (int i = size; --i >= 0;) {
				if (!(theElements[i] == null ? otherElements[i] == null
						: theElements[i].equals(otherElements[i])))
					return false;
			}
		}
	 ...
	 *                ).
	 */
	public int indexOfFromTo(Object element, int from, int to,
	...
	public boolean isSortedFromTo(int from, int to) {
		...
		for (int i = from + 1; i <= to; i++) {
			if (((Comparable) theElements[i])
					.compareTo((Comparable) theElements[i - 1]) < 0)
				return false;
		}
		return true;
	}
	 ...
	 *                ).
	 */
	public int lastIndexOfFromTo(Object element, int from, int to,
			boolean testForEquality) {
		if (size == 0)
			return -1;
		checkRangeFromTo(from, to, size);
	 ...
	 */
	public void replaceFromToWithFrom(int from, int to, ObjectArrayList other,
			int otherFrom) {
		int length = to - from + 1;
		if (length > 0) {
			checkRangeFromTo(from, to, size);
	...
	public void replaceFromToWithFromTo(int from, int to,
			ObjectArrayList other, int otherFrom, int otherTo) {
