 ...
 * picture.
 */
public abstract class AbstractDoubleList extends AbstractList implements
	 ...
	 *                <tt>index &lt; 0 || index &gt; size()</tt>).
	 */
	public void beforeInsertAllOfFromTo(int index, AbstractDoubleList other,
			int from, int to) {
	...
	public void replaceFromToWithFrom(int from, int to,
			AbstractDoubleList other, int otherFrom) {
		int length = to - from + 1;
		if (length > 0) {
			checkRangeFromTo(from, to, size());
	...
	public void replaceFromToWithFromTo(int from, int to,
			AbstractDoubleList other, int otherFrom, int otherTo) {
	...
	public void shuffleFromTo(int from, int to) {
		...
		cern.jet.random.Uniform gen = new cern.jet.random.Uniform(
				cern.jet.random.Uniform.makeDefaultGenerator());
		for (int i = from; i < to; i++) {
