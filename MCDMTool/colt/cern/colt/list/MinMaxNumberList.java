...
public class MinMaxNumberList extends cern.colt.list.AbstractLongList {
	...
	public void add(long element) {
		...
		QuickBitVector.putLongFromTo(this.bits, element - this.minValue, i, i
	...
	public void addAllOfFromTo(long[] elements, int from, int to) {
		...
		for (int times = to - from + 1; --times >= 0;) {
			QuickBitVector.putLongFromTo(theBits, elements[i++] - min,
					firstBit, firstBit + bitsPerElemMinusOne);
	...
	public static int bitsPerElement(long minimum, long maximum) {
		...
		if (1 + maximum - minimum > 0) {
			bits = (int) Math.round(Math.ceil(cern.jet.math.Arithmetic.log(2, 1
					+ maximum - minimum)));
		} else {
	...
	public long getQuick(int index) {
		...
		return this.minValue
				+ QuickBitVector.getLongFromTo(this.bits, i, i
						+ this.bitsPerElement - 1);
	}
	 ...
	 * </pre>
	 */
	public void partFromTo(final int from, final int to,
			final BitVector qualificants, final int qualificantsFrom,
		...
		if (from < 0
				...
				|| to >= size
				|| qualificantsFrom < 0
				|| (qualificants != null && qualificantsFrom + width > qualificants
						.size())) {
		...
		for (int i = from; i <= to; i++, q++, p++, j += bitsPerElem) {
			if (qualificants == null || qualificants.get(q)) {
				...
				// j+bitsPerElem-1);
				part[p] = minVal
						+ QuickBitVector.getLongFromTo(theBits, j, j
								+ bitsPerElem - 1);
			}
		}
	}
	...
	protected void setUp(long minimum, long maximum, int initialCapacity) {
		...
		this.bits = QuickBitVector.makeBitVector(initialCapacity,
				this.bitsPerElement);
		this.capacity = initialCapacity;
		this.size = 0;
	}
