...
class GenericSortingTest extends Object {
	...
	public static void testRandomly(int runs) {
		...
		for (int run = 0; run < runs; run++) {
			...
			cern.colt.matrix.DoubleMatrix2D A1 = new cern.colt.matrix.impl.DenseDoubleMatrix2D(
					size, size);
			cern.colt.matrix.DoubleMatrix2D P1 = A1.viewPart(from, from, size
					- to, size - to);
			...
			cern.colt.matrix.DoubleMatrix2D P2 = A2.viewPart(from, from, size
					- to, size - to);

			int c = 0;
			cern.colt.matrix.DoubleMatrix2D S1 = cern.colt.matrix.doublealgo.Sorting.quickSort
					.sort(P1, c);
			cern.colt.matrix.DoubleMatrix2D S2 = cern.colt.matrix.doublealgo.Sorting.mergeSort
					.sort(P2, c);
