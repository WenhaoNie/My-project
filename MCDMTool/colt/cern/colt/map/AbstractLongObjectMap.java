...
public abstract class AbstractLongObjectMap extends AbstractMap {
	 ...
	 */
	public void pairsMatching(final LongObjectProcedure condition,
			final LongArrayList keyList, final ObjectArrayList valueList) {
		keyList.clear();
		valueList.clear();
	 ...
	 *            the list to be filled with values, can have any size.
	 */
	public void pairsSortedByKey(final LongArrayList keyList,
	 ...
	 *            the list to be filled with values, can have any size.
	 */
	public void pairsSortedByValue(final LongArrayList keyList,
			final ObjectArrayList valueList) {
		keys(keyList);
		values(valueList);
		...
		cern.colt.function.IntComparator comp = new cern.colt.function.IntComparator() {
			public int compare(int a, int b) {
				...
				return ab < 0 ? -1 : ab > 0 ? 1 : (k[a] < k[b] ? -1
						: (k[a] == k[b] ? 0 : 1));
