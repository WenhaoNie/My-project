...
public class Sorting extends Object {
	 ...
	 * @see java.util.Arrays
	 */
	public static int binarySearchFromTo(double[] list, double key, int from,
			int to) {
		double midVal;
		while (from <= to) {
			int mid = (from + to) / 2;
	 ...
	 * @see java.util.Arrays
	 */
	public static int binarySearchFromTo(float[] list, float key, int from,
			int to) {
		float midVal;
		while (from <= to) {
			int mid = (from + to) / 2;
	 ...
	 */
	public static int binarySearchFromTo(Object[] list, Object key, int from,
			int to, java.util.Comparator comparator) {
		Object midVal;
		while (from <= to) {
			int mid = (from + to) / 2;
			midVal = list[mid];
	 ...
	 * @see java.util.Arrays
	 */
	public static int binarySearchFromTo(short[] list, short key, int from,
			int to) {
		short midVal;
		while (from <= to) {
			int mid = (from + to) / 2;
	...
	private static void inplace_merge(int[] array, int first, int middle,
			int last) {
	...
	private static void mergeSort1(double src[], double dest[], int low,
			int high) {
		int length = high - low;
	...
	private static void mergeSort1(float src[], float dest[], int low,
			int high, FloatComparator c) {
		int length = high - low;
	...
	private static void mergeSort1(short src[], short dest[], int low,
			int high, ShortComparator c) {
		int length = high - low;
	...
	private static void mergeSort2(double a[], int fromIndex, int toIndex) {
		...
		if (numNegZeros != 0) {
			int j = new cern.colt.list.DoubleArrayList(a).binarySearchFromTo(
					0.0d, fromIndex, n - 1); // posn of ANY zero
			do {
				j--;
			} while (j >= 0 && a[j] == 0.0d);
	...
	private static void mergeSort2(float a[], int fromIndex, int toIndex) {
		...
		// Postprocessing phase: change 0.0's to -0.0's as required
		if (numNegZeros != 0) {
			int j = new cern.colt.list.FloatArrayList(a).binarySearchFromTo(
					0.0f, fromIndex, n - 1); // posn of ANY zero
			do {
				j--;
			} while (j >= 0 && a[j] == 0.0f);
