 ...
 * <td align="left" valign="top"> <tt> 5&nbsp;x&nbsp;4&nbsp;matrix<br>
	  ...
	  1236.345679&nbsp;0.000000&nbsp;&nbsp;7.000000&nbsp;-1.200000 </tt>
 * </td>
 * <td align="left" valign="top"><tt> {<br>
	  &nbsp;&nbsp;&nbsp;{&nbsp;&nbsp;&nbsp;3.000000,&nbsp;0.000000,&nbsp;-3.400000,&nbsp;&nbsp;0.000000},<br>
	  &nbsp;&nbsp;&nbsp;{&nbsp;&nbsp;&nbsp;5.100000,&nbsp;0.000000,&nbsp;&nbsp;3.012346,&nbsp;&nbsp;0.000000},<br>
 ...
 * <td align="left" valign="top"><tt>5&nbsp;x&nbsp;4&nbsp;matrix<br>
	  ...
	  &nbsp;1.24e+003&nbsp;0.00e+000&nbsp;&nbsp;7.00e+000&nbsp;-1.20e+000 </tt>
 * </td>
 * <td align="left" valign="top"><tt>{<br>
	  &nbsp;&nbsp;&nbsp;{&nbsp;3.00e+000,&nbsp;0.00e+000,&nbsp;-3.40e+000,&nbsp;&nbsp;0.00e+000},<br>
	  &nbsp;&nbsp;&nbsp;{&nbsp;5.10e+000,&nbsp;0.00e+000,&nbsp;&nbsp;3.01e+000,&nbsp;&nbsp;0.00e+000},<br>
...
public class Formatter extends AbstractFormatter {
	...
	public static void demo1() {
		...
		double[][] values = { { 3, 0, -3.4, 0 }, { 5.1, 0, +3.0123456789, 0 },
				{ 16.37, 0.0, 2.5, 0 }, { -16.3, 0, -3.012345678E-4, -1 },
				{ 1236.3456789, 0, 7, -1.2 } };
		String[] formats = { "%G", "%1.10G", "%f", "%1.2f", "%0.2e", null };

		// now the processing
		int size = formats.length;
		DoubleMatrix2D matrix = cern.colt.matrix.DoubleFactory2D.dense
				.make(values);
	...
	public static void demo3(int size, double value) {
		...
		String s;
		StringBuffer buf;
		DoubleMatrix2D matrix = cern.colt.matrix.DoubleFactory2D.dense.make(
				size, size, value);
		...
		timer.reset().start();
		cern.colt.matrix.impl.Former format = new cern.colt.matrix.impl.FormerFactory()
				.create("%G");
		buf = new StringBuffer();
		for (int i = size; --i >= 0;) {
			for (int j = size; --j >= 0;) {
	...
	public static void demo4() {
		...
		String[] rowNames = { "SunJDK1.2.2 classic", "IBMJDK1.1.8",
				"SunJDK1.3 Hotspot", "other1", "other2" };
		...
		DoubleMatrix2D matrix = cern.colt.matrix.DoubleFactory2D.dense
				.make(values);
		System.out.println("\n\n"
				+ new Formatter("%G").toTitleString(matrix, rowNames,
						columnNames, "rowAxis", "colAxis",
	...
	public static void demo5() {
		...
		System.out.println(new Formatter("%G").toTitleString(
				cern.colt.matrix.DoubleFactory2D.dense.make(values), rowNames,
	...
	public static void demo6() {
		...
		System.out.println(new Formatter()
				.toString(cern.colt.matrix.DoubleFactory2D.dense.make(values)));
		System.out.println(new Formatter().toTitleString(
				cern.colt.matrix.DoubleFactory2D.dense.make(values), rowNames,
				columnNames, "vendor", "density", "title"));
	}
	...
	public static void demo7() {
		 ...
		 * 0, 7, -1.2} };
		 */
		double[][] values = { { 5, 10, 20, 40 }, { 7, 8, 6, 7 },
				{ 12, 10, 20, 19 }, { 3, 1, 5, 6 } };
		...
		System.out.println(new Formatter(format).toTitleString(
				cern.colt.matrix.DoubleFactory2D.dense.make(values), rowNames,
				columnNames, rowAxisName, columnAxisName, title, aggr));
	 ...
	 */
	protected String toTitleString(DoubleMatrix2D matrix, String[] rowNames,
		...
		return new cern.colt.matrix.objectalgo.Formatter().toTitleString(
				cern.colt.matrix.ObjectFactory2D.dense.make(s), rowNames,
	 ...
	 */
	public String toTitleString(DoubleMatrix2D matrix, String[] rowNames,
			String[] columnNames, String rowAxisName, String columnAxisName,
			String title, hep.aida.bin.BinFunction1D[] aggr) {
		if (matrix.size() == 0)
			return "Empty matrix";
		if (aggr == null || aggr.length == 0)
			return toTitleString(matrix, rowNames, columnNames, rowAxisName,
		...
		cern.colt.matrix.doublealgo.Statistic.aggregate(matrix, aggr, colStats); // aggregate
																					...
																					// a
																					// time
		cern.colt.matrix.doublealgo.Statistic.aggregate(matrix.viewDice(),
				aggr, rowStats.viewDice()); // aggregate an entire row at a time

		// turn into strings
		// tmp holds "matrix" plus "colStats" below (needed so that numbers in a
		// columns can be decimal point aligned)
		DoubleMatrix2D tmp = matrix.like(matrix.rows() + aggr.length,
				matrix.columns());
		tmp.viewPart(0, 0, matrix.rows(), matrix.columns()).assign(matrix);
		tmp.viewPart(matrix.rows(), 0, aggr.length, matrix.columns()).assign(
				colStats);
		colStats = null;
		...
		// aggregations
		cern.colt.matrix.ObjectMatrix2D allStats = cern.colt.matrix.ObjectFactory2D.dense
				.make(matrix.rows() + aggr.length, matrix.columns()
						+ aggr.length + 1);
		...
		if (columnNames != null) {
			cern.colt.list.ObjectArrayList list = new cern.colt.list.ObjectArrayList(
					columnNames);
			list.add("|");
		...
		String s = new cern.colt.matrix.objectalgo.Formatter().toTitleString(
				allStats, rowNames, columnNames, rowAxisName, columnAxisName,
				title);
		...
		int last = s.length() + 1;
		int secondLast = last;
		int v = Math.max(0, rowAxisName == null ? 0 : rowAxisName.length()
				- matrix.rows() - aggr.length);
		...
		buf.insert(secondLast, rowSeparator
				+ repeat('-', secondLast - last - 1));
	 ...
	 * @see hep.aida.bin.BinFunctions1D
	 */
	public String toTitleString(DoubleMatrix3D matrix, String[] sliceNames,
	 ...
	 * @return the matrix converted to a string.
	 */
	private String xtoTitleString(DoubleMatrix3D matrix, String[] sliceNames,
			String[] rowNames, String[] columnNames, String sliceAxisName,
			String rowAxisName, String columnAxisName, String title) {
		if (matrix.size() == 0)
			return "Empty matrix";
