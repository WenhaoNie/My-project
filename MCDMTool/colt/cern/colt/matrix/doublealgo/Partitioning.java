...
public class Partitioning extends Object {
	 ...
	 */
	public static void partition(DoubleMatrix2D matrix, int[] rowIndexes,
			int rowFrom, int rowTo, int column, final double[] splitters,
		...
		cern.colt.Partitioning.genericPartition(rowFrom, rowTo, splitFrom,
				splitTo, splitIndexes, comp, comp2, comp3, swapper);
	}
	 ...
	 *         splitters.
	 */
	public static DoubleMatrix2D partition(DoubleMatrix2D matrix, int column,
			final double[] splitters, int[] splitIndexes) {
		int rowFrom = 0;
		...
		partition(matrix, rowIndexes, rowFrom, rowTo, column, splitters,
				splitFrom, splitTo, splitIndexes);
	 ...
	 * </table>
	 */
	private static void xPartitionOld(DoubleMatrix2D matrix,
			DoubleMatrix1D column, int from, int to, double[] splitters,
			int splitFrom, int splitTo, int[] splitIndexes) {
		/*
		 ...
		 * splitIndexes[i++] = from; return; }
		 * 
		 * // Choose a partition (pivot) index, m // Ideally, the pivot should
		 * be the median, because a median splits a list into two equal sized
		 * sublists. // However, computing the median is expensive, so we use an
		 * approximation. int medianIndex; if (splitFrom==splitTo) { // we don't
		 * really have a choice medianIndex = splitFrom; } else { // we do have
		 * a choice int m = (from+to) / 2; // Small arrays, middle element int
		 * len = to-from+1; if (len > SMALL) { int l = from; int n = to; if (len
		 * > MEDIUM) { // Big arrays, pseudomedian of 9 int s = len/8; l =
		 * med3(column, l, l+s, l+2*s); m = med3(column, m-s, m, m+s); n =
		 * med3(column, n-2*s, n-s, n); } m = med3(column, l, m, n); //
		 * Mid-size, pseudomedian of 3 }
		 * 
		 * // Find the splitter closest to the pivot, i.e. the splitter that
		 * best splits the list into two equal sized sublists. medianIndex =
		 * cern
		 * .colt.Sorting.binarySearchFromTo(splitters,column.getQuick(m),splitFrom
		 * ,splitTo); if (medianIndex < 0) medianIndex = -medianIndex - 1; //
		 * not found if (medianIndex > splitTo) medianIndex = splitTo; // not
		 * found, one past the end
		 * 
		 * } splitter = splitters[medianIndex];
		 * 
		 * // Partition the list according to the splitter, i.e. // Establish
		 * invariant: list[i] < splitter <= list[j] for i=from..medianIndex and
		 * j=medianIndex+1 .. to int splitIndex =
		 * xPartitionOld(matrix,column,from,to,splitter);
		 * splitIndexes[medianIndex] = splitIndex;
		 * 
		 * // Optimization: Handle special cases to cut down recursions. if
		 * (splitIndex < from) { // no element falls into this bin // all bins
		 * with splitters[i] <= splitter are empty int i = medianIndex-1; while
		 * (i>=splitFrom && (!(splitter < splitters[i]))) splitIndexes[i--] =
		 * splitIndex; splitFrom = medianIndex+1; } else if (splitIndex >= to) {
		 * // all elements fall into this bin // all bins with splitters[i] >=
		 * splitter are empty int i = medianIndex+1; while (i<=splitTo &&
		 * (!(splitter > splitters[i]))) splitIndexes[i++] = splitIndex; splitTo
		 * = medianIndex-1; }
		 * 
		 * // recursively partition left half if (splitFrom <= medianIndex-1) {
		 * xPartitionOld(matrix, column, from, splitIndex, splitters, splitFrom,
		 * medianIndex-1, splitIndexes); }
		 * 
		 * // recursively partition right half if (medianIndex+1 <= splitTo) {
	 ...
	 * Note that arguments are not checked for validity.
	 */
	private static int xPartitionOld(DoubleMatrix2D matrix,
			DoubleMatrix1D column, int from, int to, double splitter) {
		/*
