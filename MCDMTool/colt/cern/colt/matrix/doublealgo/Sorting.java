...
public class Sorting extends cern.colt.PersistentObject {
	 ...
	 */
	public DoubleMatrix1D sort(final DoubleMatrix1D vector,
			final cern.colt.function.DoubleComparator c) {
		int[] indexes = new int[vector.size()]; // row indexes to reorder
												// instead of matrix itself
		for (int i = indexes.length; --i >= 0;)
			indexes[i] = i;
	 ...
	 * 
	 * <pre>
	 * // sort 10000 x 1000 matrix by sum of logarithms in a row (i.e. by geometric
	 * // mean)
	 * DoubleMatrix2D matrix = new DenseDoubleMatrix2D(10000, 1000);
	 * matrix.assign(new cern.jet.random.engine.MersenneTwister()); // initialized
	 * 																// randomly
	 * cern.jet.math.Functions F = cern.jet.math.Functions.functions; // alias for
	 * 																// convenience
	 * 
	 ...
	 */
	public DoubleMatrix2D sort(final DoubleMatrix2D matrix,
	 ...
	 * 
	 * <pre>
	 * // sort 10000 x 1000 matrix by median or by sum of logarithms in a row (i.e. by
	 * // geometric mean)
	 * DoubleMatrix2D matrix = new DenseDoubleMatrix2D(10000, 1000);
	 * matrix.assign(new cern.jet.random.engine.MersenneTwister()); // initialized
	 * 																// randomly
	 ...
	 *         matrix is left unaffected.</b>
	 */
	public DoubleMatrix2D sort(DoubleMatrix2D matrix,
	...
	public DoubleMatrix3D sort(DoubleMatrix3D matrix, int row, int column) {
		if (row < 0 || row >= matrix.rows())
			throw new IndexOutOfBoundsException("row=" + row + ", matrix="
					+ Formatter.shape(matrix));
		if (column < 0 || column >= matrix.columns())
			throw new IndexOutOfBoundsException("column=" + column
					+ ", matrix=" + Formatter.shape(matrix));
	 ...
	 *         the original matrix is left unaffected.</b>
	 */
	public DoubleMatrix3D sort(final DoubleMatrix3D matrix,
			final DoubleMatrix2DComparator c) {
		int[] sliceIndexes = new int[matrix.slices()]; // indexes to reorder
														// instead of matrix
														// itself
		for (int i = sliceIndexes.length; --i >= 0;)
	...
	public static void zdemo5(int rows, int columns, boolean print) {
		...
		DoubleMatrix2D A = cern.colt.matrix.DoubleFactory2D.dense.make(rows,
				columns);
		...
		if (print) {
			...
			hep.aida.bin.BinFunction1D[] funs = {
			...
			System.out.println("first part of sorted result = \n"
					+ new cern.colt.matrix.doublealgo.Formatter("%G")
							.toTitleString(A.viewPart(0, 0, r, columns),
									rowNames, columnNames, null, null, null,
									funs));
		}
		...
		cern.colt.matrix.doublealgo.DoubleMatrix1DComparator fun = new cern.colt.matrix.doublealgo.DoubleMatrix1DComparator() {
			public int compare(DoubleMatrix1D x, DoubleMatrix1D y) {
				double a = cern.colt.matrix.doublealgo.Statistic.bin(x)
						.median();
				double b = cern.colt.matrix.doublealgo.Statistic.bin(y)
						.median();
				// double a = x.aggregate(F.plus,F.log);
				// double b = y.aggregate(F.plus,F.log);
				return a < b ? -1 : (a == b) ? 0 : 1;
	...
	public static void zdemo6() {
		Sorting sort = quickSort;
		double[][] values = { { 3, 7, 0 }, { 2, 1, 0 }, { 2, 2, 0 },
				{ 1, 8, 0 }, { 2, 5, 0 }, { 7, 0, 0 }, { 2, 3, 0 },
				{ 1, 0, 0 }, { 4, 0, 0 }, { 2, 0, 0 } };
