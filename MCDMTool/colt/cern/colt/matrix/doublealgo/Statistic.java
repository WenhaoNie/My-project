 ...
 * {@link hep.aida.bin}, in particular {@link hep.aida.bin.DynamicBin1D}.
 * <p>
 * Examples:
...
public class Statistic extends Object {
	...
	public static final VectorVectorFunction EUCLID = new VectorVectorFunction() {
		public final double apply(DoubleMatrix1D a, DoubleMatrix1D b) {
			return Math
					.sqrt(a.aggregate(b, F.plus, F.chain(F.square, F.minus)));
		}
	};
	...
	public interface VectorVectorFunction {
		 ...
		 */
		abstract public double apply(cern.colt.matrix.DoubleMatrix1D x,
	...
	public static DoubleMatrix2D aggregate(DoubleMatrix2D matrix,
			hep.aida.bin.BinFunction1D[] aggr, DoubleMatrix2D result) {
	 ...
	 * math definition</A> and <A HREF=
	 * "http://www.stat.berkeley.edu/users/stark/SticiGui/Text/gloss.htm#correlation_coef"
	...
	public static DoubleMatrix2D covariance(DoubleMatrix2D matrix) {
		...
		DoubleMatrix2D covariance = new cern.colt.matrix.impl.DenseDoubleMatrix2D(
				columns, columns);
	 ...
	 */
	public static hep.aida.IHistogram2D cube(DoubleMatrix1D x,
		...
		cern.colt.list.DoubleArrayList sorted = new cern.colt.list.DoubleArrayList(
				vals);
		...
		distinct.trimToSize();
		hep.aida.IAxis xaxis = new hep.aida.ref.VariableAxis(
				distinct.elements());
		...
		if (distinct.size() > 0)
			distinct.add(distinct.get(distinct.size() - 1) + epsilon);
		distinct.trimToSize();
		hep.aida.IAxis yaxis = new hep.aida.ref.VariableAxis(
				distinct.elements());

		hep.aida.IHistogram2D histo = new hep.aida.ref.Histogram2D("Cube",
				xaxis, yaxis);
	 ...
	 *             .
	 */
	public static hep.aida.IHistogram3D cube(DoubleMatrix1D x,
		...
		if (distinct.size() > 0)
			distinct.add(distinct.get(distinct.size() - 1) + epsilon);
		distinct.trimToSize();
		hep.aida.IAxis zaxis = new hep.aida.ref.VariableAxis(
				distinct.elements());

		hep.aida.IHistogram3D histo = new hep.aida.ref.Histogram3D("Cube",
				xaxis, yaxis, zaxis);
	...
	public static void demo3(VectorVectorFunction norm) {
		double[][] values = { { -0.9611052, -0.25421095 },
				{ 0.4308269, -0.69932648 }, { -1.2071029, 0.62030596 },
				{ 1.5345166, 0.02135884 }, { -1.1341542, 0.20388430 } };
	...
	public static DoubleMatrix2D distance(DoubleMatrix2D matrix,
		...
		int columns = matrix.columns();
		DoubleMatrix2D distance = new cern.colt.matrix.impl.DenseDoubleMatrix2D(
				columns, columns);
	 ...
	 */
	public static hep.aida.IHistogram1D histogram(hep.aida.IHistogram1D histo,
	...
	public static hep.aida.IHistogram3D histogram(hep.aida.IHistogram3D histo,
			DoubleMatrix1D x, DoubleMatrix1D y, DoubleMatrix1D z,
			DoubleMatrix1D weights) {
		if (x.size() != y.size() || x.size() != z.size()
				|| x.size() != weights.size())
			throw new IllegalArgumentException("vectors must have same size");
		for (int i = x.size(); --i >= 0;) {
			histo.fill(x.getQuick(i), y.getQuick(i), z.getQuick(i),
					weights.getQuick(i));
		}
		return histo;
	}
	...
	public static DoubleMatrix1D viewSample(DoubleMatrix1D matrix,
			double fraction, RandomEngine randomGenerator) {
		...
		if (randomGenerator == null)
			randomGenerator = new cern.jet.random.engine.MersenneTwister(
		...
		int n = ncols;
		int N = matrix.size();
		cern.jet.random.sampling.RandomSampler.sample(n, N, n, 0, selected, 0,
				randomGenerator);
	 ...
	 * <tr valign="top" align="center">
	 * <td>
	 * <div align="left"><tt>matrix</tt></div></td>
	 * <td>
	...
	public static DoubleMatrix2D viewSample(DoubleMatrix2D matrix,
			double rowFraction, double columnFraction,
	...
	public static DoubleMatrix3D viewSample(DoubleMatrix3D matrix,
			double sliceFraction, double rowFraction, double columnFraction,
			RandomEngine randomGenerator) {
		// check preconditions and allow for a little tolerance
		double epsilon = 1e-09;
	...
	private static DoubleMatrix2D xdistanceOld(DoubleMatrix2D matrix, int norm) {
		 ...
		 * cern.colt.matrix.impl.DenseDoubleMatrix2D(columns,columns);
		 * 
		 * // cache views DoubleMatrix1D[] cols = new DoubleMatrix1D[columns];
		 * for (int i=columns; --i >= 0; ) { cols[i] = matrix.viewColumn(i); }
		 * 
		 * // setup distance function cern.jet.math.Functions F =
		 ...
		 * 
		 * // work out all permutations for (int i=columns; --i >= 0; ) { for
		 * (int j=i; --j >= 0; ) { double d = 0; if (norm==EUCLID) d =
		 * Math.sqrt(cols[i].aggregate(cols[j], F.plus, function)); else if
		 * (norm==BRAY_CURTIS) d = cols[i].aggregate(cols[j], F.plus, function)
		 * / cols[i].aggregate(cols[j], F.plus, F.plus); else if
		 * (norm==CANBERRA) d = cols[i].aggregate(cols[j], F.plus, function);
		 * else if (norm==MAXIMUM) d = cols[i].aggregate(cols[j], F.max,
		 * function); else if (norm==MANHATTAN) d = cols[i].aggregate(cols[j],
		 * F.plus, function); distance.setQuick(i,j,d);
		 * distance.setQuick(j,i,d); // symmetric } } return distance;
		 */
		return null;
	}
