...
public class BenchmarkMatrix {
	...
	protected static void bench_dgemm(String[] args) {
		...
		cern.colt.matrix.linalg.SmpBlas.allocateBlas(cpus,
 ...
 */
	 ...
	 */
	protected static Double2DProcedure fun_dgemm(final boolean transposeA,
		...
		return new Double2DProcedure() {
			...
			public void setParameters(DoubleMatrix2D G, DoubleMatrix2D H) {
				...
				D = new cern.colt.matrix.impl.DenseDoubleMatrix2D(A.rows(),
						A.columns()).assign(0.5);
				C = D.copy();
				B = D.copy();
			}
			...
			public void apply(cern.colt.Timer timer) {
				cern.colt.matrix.linalg.SmpBlas.smpBlas.dgemm(transposeA,
						transposeB, 1, A, B, 0, C);
			}
	...
	protected static Double2DProcedure fun_dgemv(final boolean transposeA) {
		return new Double2DProcedure() {
			...
			public void apply(cern.colt.Timer timer) {
				cern.colt.matrix.linalg.SmpBlas.smpBlas.dgemv(transposeA, 1, A,
						B.viewRow(0), 0, C.viewRow(0));
			}
		...
		return new Double2DProcedure() {
			...
			public void setParameters(DoubleMatrix2D A, DoubleMatrix2D B) {
				if (k < 0) { // must be nonsingular for inversion
					if (!cern.colt.matrix.linalg.Property.ZERO
							.isDiagonallyDominantByRow(A)
							|| !cern.colt.matrix.linalg.Property.ZERO
									.isDiagonallyDominantByColumn(A)) {
						cern.colt.matrix.linalg.Property.ZERO
								.generateNonSingular(A);
					}
	...
	protected static Double2DProcedure funAssignLog() {
		return new Double2DProcedure() {
			...
			public void apply(cern.colt.Timer timer) {
				cern.colt.matrix.linalg.SmpBlas.smpBlas.assign(A,
	...
	protected static Double2DProcedure funAssignPlusMult() {
		return new Double2DProcedure() {
			...
			public void apply(cern.colt.Timer timer) {
				cern.colt.matrix.linalg.SmpBlas.smpBlas.assign(A, B,
						cern.jet.math.Functions.plusMult(0.5));
			}
		...
		return new Double2DProcedure() {
			...
			public void apply(cern.colt.Timer timer) {
				cern.colt.matrix.doublealgo.Statistic
						.correlation(cern.colt.matrix.doublealgo.Statistic
								.covariance(A));
			}
	...
	protected static Double2DProcedure funElementwiseMult() {
		return new Double2DProcedure() {
			...
			public void apply(cern.colt.Timer timer) {
				cern.colt.matrix.linalg.SmpBlas.smpBlas.assign(A,
						cern.jet.math.Functions.mult(0.5));
			}
		};
	}
	...
	protected static Double2DProcedure funLUDecompose() {
		return new Double2DProcedure() {
			cern.colt.matrix.linalg.LUDecompositionQuick lu = new cern.colt.matrix.linalg.LUDecompositionQuick(
					0);
		...
		return new Double2DProcedure() {
			...
			cern.colt.function.Double9Function function = new cern.colt.function.Double9Function() {
				public final double apply(double a00, double a01, double a02,
						double a10, double a11, double a12, double a20,
						double a21, double a22) {
					return alpha * a11 + beta
							* (a00 + a10 + a20 + a01 + a21 + a02 + a12 + a22);
				}
			};
 ...
 */
	...
	protected static DoubleFactory2D getFactory(String type) {
		...
		String s = "type=" + type
				+ " is unknown. Use one of {dense,sparse,rowCompressed}";
	...
	protected static boolean handle(String[] params) {
		...
		if (cmd.equals("dgemm"))
			...
			if (fun != null) {
				...
				success = false;
				String s = "Command=" + params[0]
						+ " is illegal or unknown. Should be one of "
						+ commands() + "followed by appropriate parameters.\n"
						+ usage() + "\nIgnoring this line.\n";
	...
	public static void main(String[] args) {
		...
		if (!args[0].equals("-file")) { // interactive mode, commands supplied
										// via java class args
			System.out.println("\n\nExecuting command = "
					+ new cern.colt.list.ObjectArrayList(args) + " ...");
			handle(args);
			...
			try {
				...
				while ((token = stream.nextToken()) != stream.TT_EOF) { // while
																		...
																		// of
																		// file
					if (token == stream.TT_EOL) { // execute a command line at a
													// time
						...
						String word;
						cern.colt.matrix.impl.Former formatter = new cern.colt.matrix.impl.FormerFactory()
								.create("%G");
				 ...
				 */
				String line;
				while ((line = reader.readLine()) != null) { // while not end of
																// file
		...
		System.out.println("\nProgram execution took a total of "
	 ...
	 */
	protected static void run(double minSeconds, String title,
			Double2DProcedure function, String[] types, int[] sizes,
			double[] densities) {
		...
		DoubleMatrix3D timings = DoubleFactory3D.dense.make(types.length,
				sizes.length, densities.length);
				...
				for (int j = 0; j < densities.length; j++) {
					...
					if (!((k > 0 && density >= 0.1 && size >= 500))) {
						...
						function.D = null; // --> help gc before allocating new
											// mem
						DoubleMatrix2D A = factory.sample(size, size, val,
								density);
						DoubleMatrix2D B = factory.sample(size, size, val,
								density);
		...
		System.out.println(new cern.colt.matrix.doublealgo.Formatter("%1.3G")
				.toTitleString(timings, sliceNames, rowNames, colNames,
						sliceAxisName, rowAxisName, colAxisName,
						"Performance of " + title, aggr));
		/*
		 * title = "Speedup of dense over sparse"; DoubleMatrix2D speedup =
		 * cern.
		 * colt.matrix.doublealgo.Transform.div(timings.viewSlice(0).copy(),
		 * timings.viewSlice(1)); System.out.println("\n"+new
		 * cern.colt.matrix.doublealgo
		 * .Formatter("%1.3G").toTitleString(speedup,rowNames
	 ...
	 * Executes procedure repeatadly until more than minSeconds have elapsed.
	 */
	protected static void runSpecial(double minSeconds, String title,
		...
		for (int i = 0; i < sizes.length; i++) {
			...
			final DoubleFactory2D factory = (sparse ? DoubleFactory2D.sparse
		...
		System.out.println("*");
		System.out.println(new cern.colt.matrix.doublealgo.Formatter("%1.3G")
				.toTitleString(timings, rowNames, colNames, rowAxisName,
						colAxisName, title, aggr));
	...
	protected static String usage(String cmd) {
		String usage = cmd + " description: "
				...
				// String usage =
				// "Illegal arguments! Arguments to be supplied:\n" +
		...
		if (cmd.equals("pow"))
			usage = usage
					+ "\texponent = the number of times to multiply; e.g. 1000\n";
		usage = usage
