...
public class Sorting extends cern.colt.PersistentObject {
	 ...
	 */
	public ObjectMatrix2D sort(final ObjectMatrix2D matrix,
			final ObjectMatrix1DComparator c) {
		int[] rowIndexes = new int[matrix.rows()]; // row indexes to reorder
													// instead of matrix itself
		for (int i = rowIndexes.length; --i >= 0;)
			rowIndexes[i] = i;
	 ...
	 *         the original matrix is left unaffected.</b>
	 */
	public ObjectMatrix3D sort(final ObjectMatrix3D matrix,
			final ObjectMatrix2DComparator c) {
		int[] sliceIndexes = new int[matrix.slices()]; // indexes to reorder
														// instead of matrix
														// itself
		for (int i = sliceIndexes.length; --i >= 0;)
