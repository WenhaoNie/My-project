...
public class DenseDoubleMatrix1D extends DoubleMatrix1D {
	 ...
	 *             if <tt>size<0</tt>.
	 */
	protected DenseDoubleMatrix1D(int size, double[] elements, int zero,
			int stride) {
		...
		if (isNoView) {
			if (values.length != size)
				throw new IllegalArgumentException(
						"Must have same number of cells: length="
	...
	public DoubleMatrix1D assign(DoubleMatrix1D y,
		...
		if (function == cern.jet.math.Functions.mult) { // x[i] = x[i] * y[i]
			...
			for (int k = size; --k >= 0;) {
				elems[index] = function.apply(elems[index],
						otherElems[otherIndex]);
				index += s;
				otherIndex += ys;
			}
		}
	...
	public double zDotProduct(DoubleMatrix1D y, int from, int length) {
		...
		double sum = 0;
		/*
		 * // unoptimized for (int k = min; --k >= 0;) { sum += elems[i] *
		 * yElems[j]; i += s; j += ys; }
		 */
		...
		j -= ys;
		for (int k = min / 4; --k >= 0;) {
			sum += elems[i += s] * yElems[j += ys] + elems[i += s]
					* yElems[j += ys] + elems[i += s] * yElems[j += ys]
