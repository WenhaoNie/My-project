...
class TridiagonalDoubleMatrix2D extends WrapperDoubleMatrix2D {
	...
	public DoubleMatrix2D assign(DoubleMatrix2D source) {
		...
		if (source instanceof TridiagonalDoubleMatrix2D) {
			...
			System.arraycopy(other.values, 0, this.values, 0,
					this.values.length);
		...
		if (source instanceof RCDoubleMatrix2D
				|| source instanceof SparseDoubleMatrix2D) {
			assign(0);
	...
	public void setQuick(int row, int column, double value) {
		...
		// if (!isZero) throw new
		// IllegalArgumentException("Can't store non-zero value to non-tridiagonal coordinate: row="+row+", column="+column+", value="+value);
