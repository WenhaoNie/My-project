...
public abstract class AbstractFormatter extends cern.colt.PersistentObject {
	...
	public static void demo1() {
		 ...
		 * 
		 * // may not compile because of packages not included in the
		 * distribution for (int i=0; i<size; i++) {
		 * //System.out.println("\nhtmlString("
		 * +formats[i]+"):\n"+htmlStrings[i]);
		 * //System.out.println("\nhtmlSourceCode("
		 * +formats[i]+"):\n"+htmlSourceCodes[i]); }
	 ...
	 */
	protected abstract String form(AbstractMatrix1D matrix, int index,
			Former formatter);
	...
	public static String shape(AbstractMatrix3D matrix) {
		return matrix.slices() + " x " + matrix.rows() + " x "
