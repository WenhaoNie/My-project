...
public class DenseDoubleMatrix2D extends DoubleMatrix2D {
	 ...
	 */
	protected DenseDoubleMatrix2D(int rows, int columns, double[] elements,
		...
		if (this.isNoView) {
			if (values.length != rows)
				throw new IllegalArgumentException(
						"Must have same number of rows: rows=" + values.length
								+ "rows()=" + rows());
	...
	public DoubleMatrix2D assign(DoubleMatrix2D y,
		...
		} else if (function == cern.jet.math.Functions.div) { // x[i] = x[i] /
																// y[i]
			for (int row = rows; --row >= 0;) {
				for (int i = index, j = otherIndex, column = columns; --column >= 0;) {
					elems[i] /= otherElems[j];
	...
	public double getQuick(int row, int column) {
		...
		// manually inlined:
		return elements[rowZero + row * rowStride + columnZero + column
				* columnStride];
	}
	...
	public void zAssign8Neighbors(DoubleMatrix2D B,
		...
		for (int i = 1; i < r; i++) {
			...
			for (int j = 1; j < c; j++) {
				...
				B_elems[B11] = function.apply(a00, a01, a02, a10, a11, a12,
						a20, a21, a22);
				B11 += B_cs;
	...
	public DoubleMatrix1D zMult(DoubleMatrix1D y, DoubleMatrix1D z,
		...
		for (int row = rows; --row >= 0;) {
			...
			/*
			 * // not loop unrolled for (int i=indexA, j=indexY, column=columns;
			 * --column >= 0; ) { sum += AElems[i] * yElems[j]; i += As; j +=
			 * ys; }
			 */
			...
			for (int k = cols / 4; --k >= 0;) {
				sum += AElems[i += As] * yElems[j += ys] + AElems[i += As]
						* yElems[j += ys] + AElems[i += As] * yElems[j += ys]
	...
	public DoubleMatrix2D zMult(DoubleMatrix2D B, DoubleMatrix2D C,
		...
		if (B instanceof SparseDoubleMatrix2D || B instanceof RCDoubleMatrix2D) {
			...
			/*
			 * final RCDoubleMatrix2D transB = new
			 * RCDoubleMatrix2D(B.columns,B.rows); B.forEachNonZero( new
			 * cern.colt.function.IntIntDoubleFunction() { public double
			 * apply(int i, int j, double value) { transB.setQuick(j,i,value);
			 * return value; } } );
			 * 
		...
		for (; --blocks >= 0;) {
			...
			for (int j = p; --j >= 0;) {
				...
				for (int i = m_optimal; --i >= 0;) {
					...
					/*
					 * // not unrolled: for (int k = n; --k >= 0; ) { //s +=
					 * getQuick(i,k) * B.getQuick(k,j); s += AElems[kA] *
					 * BElems[kB]; kB += rB; kA += cA; }
					 */
