...
public abstract class AbstractMatrix2D extends AbstractMatrix {
	...
	protected void checkBox(int row, int column, int height, int width) {
		if (column < 0 || width < 0 || column + width > columns || row < 0
				|| height < 0 || row + height > rows)
			throw new IndexOutOfBoundsException(toStringShort() + ", column:"
					+ column + ", row:" + row + " ,width:" + width
					+ ", height:" + height);
	}
	...
	public void checkShape(AbstractMatrix2D B, AbstractMatrix2D C) {
		if (columns != B.columns || rows != B.rows || columns != C.columns
				|| rows != C.rows)
			throw new IllegalArgumentException("Incompatible dimensions: "
	 ...
	 */
	protected void setUp(int rows, int columns, int rowZero, int columnZero,
	...
	protected AbstractMatrix2D vStrides(int rowStride, int columnStride) {
		if (rowStride <= 0 || columnStride <= 0)
			throw new IndexOutOfBoundsException("illegal strides: " + rowStride
					+ ", " + columnStride);
		this.rowStride *= rowStride;
		this.columnStride *= columnStride;
		if (this.rows != 0)
