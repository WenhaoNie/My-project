...
public class DenseDoubleMatrix3D extends DoubleMatrix3D {
	...
	public DenseDoubleMatrix3D(double[][][] values) {
		this(values.length, (values.length == 0 ? 0 : values[0].length),
				(values.length == 0 ? 0 : values[0].length == 0 ? 0
						: values[0][0].length));
		assign(values);
	}
	 ...
	 *             if <tt>slices<0 || rows<0 || columns<0</tt>.
	 */
	protected DenseDoubleMatrix3D(int slices, int rows, int columns,
			double[] elements, int sliceZero, int rowZero, int columnZero,
			int sliceStride, int rowStride, int columnStride) {
		setUp(slices, rows, columns, sliceZero, rowZero, columnZero,
		...
		if (this.isNoView) {
			if (values.length != slices)
				throw new IllegalArgumentException(
						"Must have same number of slices: slices="
								+ values.length + "slices()=" + slices());
	...
	public double getQuick(int slice, int row, int column) {
		...
		// IndexOutOfBoundsException("slice:"+slice+", row:"+row+", column:"+column);
		// return elements[index(slice,row,column)];
		// manually inlined:
		return elements[sliceZero + slice * sliceStride + rowZero + row
				* rowStride + columnZero + column * columnStride];
	}
	...
	protected int index(int slice, int row, int column) {
		...
		// manually inlined:
		return sliceZero + slice * sliceStride + rowZero + row * rowStride
	 ...
	 */
	protected DoubleMatrix2D like2D(int rows, int columns, int rowZero,
			int columnZero, int rowStride, int columnStride) {
		return new DenseDoubleMatrix2D(rows, columns, this.elements, rowZero,
				columnZero, rowStride, columnStride);
	}
	...
	public void setQuick(int slice, int row, int column, double value) {
		...
		elements[sliceZero + slice * sliceStride + rowZero + row * rowStride
				+ columnZero + column * columnStride] = value;
	}
	 ...
	 * @return a new view.
	 */
	protected DoubleMatrix3D viewSelectionLike(int[] sliceOffsets,
			int[] rowOffsets, int[] columnOffsets) {
		return new SelectedDenseDoubleMatrix3D(this.elements, sliceOffsets,
				rowOffsets, columnOffsets, 0);
	}
		...
		for (int k = 1; k < slices - 1; k++) {
			...
			for (int i = 1; i < r; i++) {
				...
				for (int j = 1; j < c; j++) {
					...
					B_elems[B11] = function.apply(a000, a001, a002, a010, a011,
							a012, a020, a021, a022,
