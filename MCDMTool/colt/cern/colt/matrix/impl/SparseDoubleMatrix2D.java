...
public class SparseDoubleMatrix2D extends DoubleMatrix2D {
	 ...
	 * to memory usage see {@link cern.colt.map.OpenIntDoubleHashMap}.
	 * 
	 * @param rows
	 *            the number of rows the matrix shall have.
	 * @param columns
	 ...
	 *             .
	 */
	public SparseDoubleMatrix2D(int rows, int columns, int initialCapacity,
	 ...
	 */
	protected SparseDoubleMatrix2D(int rows, int columns,
			AbstractIntDoubleMap elements, int rowZero, int columnZero,
	...
	public DoubleMatrix2D assign(cern.colt.function.DoubleFunction function) {
		if (this.isNoView && function instanceof cern.jet.math.Mult) { // x[i] =
																		// mult*x[i]
	...
	public DoubleMatrix2D assign(final DoubleMatrix2D y,
		...
		if (function == cern.jet.math.Functions.mult) { // x[i] = x[i] * y[i]
			this.elements
					.forEachPair(new cern.colt.function.IntDoubleProcedure() {
	...
	public double getQuick(int row, int column) {
		...
		// manually inlined:
		return this.elements.get(rowZero + row * rowStride + columnZero
				+ column * columnStride);
	}
	...
	public void setQuick(int row, int column, double value) {
		...
		// manually inlined:
		int index = rowZero + row * rowStride + columnZero + column
				* columnStride;
	...
	protected DoubleMatrix2D viewSelectionLike(int[] rowOffsets,
			int[] columnOffsets) {
		return new SelectedSparseDoubleMatrix2D(this.elements, rowOffsets,
				columnOffsets, 0);
	}
	...
	public DoubleMatrix1D zMult(DoubleMatrix1D y, DoubleMatrix1D z,
		...
		this.elements.forEachPair(new cern.colt.function.IntDoubleProcedure() {
			public boolean apply(int key, double value) {
				...
				zElements[zi + zStride * i] += value
						* yElements[yi + yStride * j];
