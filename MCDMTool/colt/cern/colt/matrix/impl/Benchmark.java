...
class Benchmark {
	...
	public static void benchmark(int runs, int size, String kind,
			boolean print, int initialCapacity, double minLoadFactor,
			double maxLoadFactor, double percentNonZero) {
		...
		if (kind.equals("sparse"))
			matrix = new SparseDoubleMatrix2D(size, size, initialCapacity,
		...
		double value = 2;
		DoubleMatrix2D tmp = DoubleFactory2D.dense.sample(matrix.rows(),
				matrix.columns(), value, percentNonZero);
		matrix.assign(tmp);
		tmp = null;
		/*
		...
		/*
		 * timer1.start(); for (int i=0; i<runs; i++) { LUDecomposition LU = new
		 * LUDecomposition(matrix); } timer1.stop(); timer1.display();
		 * 
		 * { Jama.Matrix jmatrix = new Jama.Matrix(matrix.toArray());
		 * timer2.start(); for (int i=0; i<runs; i++) { Jama.LUDecomposition LU
		 * = new Jama.LUDecomposition(jmatrix); } timer2.stop();
		 * timer2.display(); }
		 */
		System.out.println("\ntesting...");
		if (print)
 ...
 */
	...
	public static void main(String args[]) {
		...
		benchmark(runs, size, kind, print, initialCapacity, minLoadFactor,
				maxLoadFactor, initialValue);
	}
}
