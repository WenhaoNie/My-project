...
public abstract class ObjectMatrix2D extends AbstractMatrix2D {
	...
	public Object aggregate(ObjectMatrix2D other,
		...
		if (size() == 0)
			return null;
		Object a = f.apply(getQuick(rows - 1, columns - 1),
				other.getQuick(rows - 1, columns - 1));
		int d = 1; // last cell already done
		for (int row = rows; --row >= 0;) {
			for (int column = columns - d; --column >= 0;) {
				a = aggr.apply(
						a,
						f.apply(getQuick(row, column),
								other.getQuick(row, column)));
			}
			d = 0;
		}
		return a;
		...
		for (int row = rows; --row >= 0;) {
			for (int column = columns; --column >= 0;) {
				setQuick(
						row,
						column,
						function.apply(getQuick(row, column),
	...
	public boolean equals(Object otherObj, boolean testForEquality) { // delta
		...
		if (!testForEquality) {
			...
			for (int row = rows; --row >= 0;) {
				for (int column = columns; --column >= 0;) {
					if (!(getQuick(row, column) == null ? other.getQuick(row,
							column) == null : getQuick(row, column).equals(
							other.getQuick(row, column))))
						return false;
				}
			}
	 ...
	 * @return a new view.
	 */
	protected abstract ObjectMatrix2D viewSelectionLike(int[] rowOffsets,
			int[] columnOffsets);
	 ...
	 * {@link cern.colt.matrix.objectalgo.Sorting#sort(ObjectMatrix2D,int)}. For
