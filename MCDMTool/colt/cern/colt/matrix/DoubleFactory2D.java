...
public class DoubleFactory2D extends cern.colt.PersistentObject {
	...
	public DoubleMatrix2D ascending(int rows, int columns) {
		...
		return descending(rows, columns).assign(
				F.chain(F.neg, F.minus(columns * rows)));
	}
	 ...
	 * 
	 * <pre>
	 * DoubleMatrix2D[][] parts1 = { { null, make(2, 2, 1), null },
	 * 		{ make(4, 4, 2), null, make(4, 3, 3) }, { null, make(2, 2, 4), null } };
	 ...
	 * 
	 * <pre>
	 * DoubleMatrix2D[][] parts3 = { { identity(3), null, },
	 ...
	 * 
	 * <pre>
	 * DoubleMatrix2D[][] parts2 = { { null, make(2, 2, 1), null },
	 * 		{ make(4, 4, 2), null, make(4, 3, 3) }, { null, make(2, 3, 4), null } };
	...
	public DoubleMatrix2D compose(DoubleMatrix2D[][] parts) {
		...
		for (int row = 0; row < rows; row++) {
			...
			for (int column = 0; column < columns; column++) {
				DoubleMatrix2D part = parts[row][column];
				if (part != null) {
					matrix.viewPart(r, c, part.rows(), part.columns()).assign(
							part);
				}
				c += maxWidths[column];
			}
	...
	public DoubleMatrix2D composeDiagonal(DoubleMatrix2D A, DoubleMatrix2D B,
			DoubleMatrix2D C) {
		DoubleMatrix2D diag = make(A.rows() + B.rows() + C.rows(), A.columns()
				+ B.columns() + C.columns());
		diag.viewPart(0, 0, A.rows(), A.columns()).assign(A);
		diag.viewPart(A.rows(), A.columns(), B.rows(), B.columns()).assign(B);
		diag.viewPart(A.rows() + B.rows(), A.columns() + B.columns(), C.rows(),
				C.columns()).assign(C);
		return diag;
	}
	...
	public void decompose(DoubleMatrix2D[][] parts, DoubleMatrix2D matrix) {
		...
		for (int row = 0; row < rows; row++) {
			...
			for (int column = 0; column < columns; column++) {
				DoubleMatrix2D part = parts[row][column];
				if (part != null) {
					part.assign(matrix.viewPart(r, c, part.rows(),
							part.columns()));
				}
				c += maxWidths[column];
			}
	...
	public void demo2() {
		 ...
		 * }; System.out.println("\n"+make(parts3));
		 * //System.out.println("\n"+cern
		 * .colt.matrixpattern.Converting.toHTML(make(parts3).toString()));
		 * 
		 ...
		 * System.out.println("\n"+make(parts4));
		 * //System.out.println("\n"+cern.
		 * colt.matrixpattern.Converting.toHTML(make(parts4).toString()));
		 */
	}
	 ...
	 * </pre>
	 */
	public DoubleMatrix2D repeat(DoubleMatrix2D A, int rowRepeat,
			int columnRepeat) {
		int r = A.rows();
		int c = A.columns();
	 ...
	 */
	public DoubleMatrix2D sample(int rows, int columns, double value,
	 ...
	 * @see cern.jet.random.sampling.RandomSampler
	 */
	public DoubleMatrix2D sample(DoubleMatrix2D matrix, double value,
			double nonZeroFraction) {
