...
public class SeqBlas implements Blas {
	/**
	 * Little trick to allow for "aliasing", that is, renaming this class. Time
	 * and again writing code like
	 * <p>
	...
	public void assign(DoubleMatrix2D A,
			cern.colt.function.DoubleFunction function) {
		A.assign(function);
	}
	...
	public void dgemm(boolean transposeA, boolean transposeB, double alpha,
			DoubleMatrix2D A, DoubleMatrix2D B, double beta, DoubleMatrix2D C) {
		A.zMult(B, C, alpha, beta, transposeA, transposeB);
	}

	public void dgemv(boolean transposeA, double alpha, DoubleMatrix2D A,
			DoubleMatrix1D x, double beta, DoubleMatrix1D y) {
	...
	public void dsymv(boolean isUpperTriangular, double alpha,
		...
		if (size != x.size() || size != y.size()) {
			throw new IllegalArgumentException(A.toStringShort() + ", "
					+ x.toStringShort() + ", " + y.toStringShort());
		}
