...
public class QRDecomposition implements java.io.Serializable {
	 ...
	 * @return a decomposition object to access <tt>R</tt> and the Householder
	 *         vectors <tt>H</tt>, and to compute <tt>Q</tt>.
		...
		for (int k = 0; k < n; k++) {
			...
			if (nrm != 0.0) {
				...
				for (int j = k + 1; j < n; j++) {
					...
					for (int i = k; i < m; i++) {
						QR.setQuick(i, j,
								QR.getQuick(i, j) + s * QR.getQuick(i, k));
					}
	...
	public String toString() {
		...
		try {
			buf.append(String.valueOf(this
					.solve(cern.colt.matrix.DoubleFactory2D.dense.identity(QR
							.rows()))));
		} catch (IllegalArgumentException exc) {
			buf.append(unknown + exc.getMessage());
		}
