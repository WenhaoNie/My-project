...
/**
 * A low level version of {@link LUDecomposition}, avoiding unnecessary memory
 * allocation and copying. The input to <tt>decompose</tt> methods is overriden
 * with the result (LU). The input to <tt>solve</tt> methods is overriden with
 * the result (X). In addition to <tt>LUDecomposition</tt>, this class also
 * includes a faster variant of the decomposition, specialized for tridiagonal
 * (and hence also diagonal) matrices, as well as a solver tuned for vectors.
 * Its disadvantage is that it is a bit more difficult to use than
...
public class LUDecompositionQuick implements java.io.Serializable {
	...
	public void decompose(DoubleMatrix2D A) {
		...
		for (int j = 0; j < n; j++) {
			...
			if (j < m && (jj = LU.getQuick(j, j)) != 0.0) {
				...
				LU.viewColumn(j).viewPart(j + 1, m - (j + 1))
						.assign(multFunction);
			}
	...
	public void decompose(DoubleMatrix2D A, int semiBandwidth) {
		...
		if (semiBandwidth == 2) { // A is tridiagonal
			...
			for (int i = 1; i < n; i++) {
				double ei = A.getQuick(i, i) - A.getQuick(i, i - 1)
						* A.getQuick(i - 1, i);
				A.setQuick(i, i, ei);
				if (i < n - 1)
	...
	protected boolean isNonsingular(DoubleMatrix2D matrix) {
		...
		double epsilon = algebra.property().tolerance(); // consider numerical
															// instability
 ...
 */
	...
	public void solve(DoubleMatrix2D B) {
		...
		for (int k = n - 1; k >= 0; k--) {
			...
			// blocking
			if (Browk == null)
				Browk = cern.colt.matrix.DoubleFactory1D.dense
						.make(B.columns());
			Browk.assign(Brows[k]);
		...
		for (int k = 0; k < n; k++) {
			for (int i = k + 1; i < n; i++) {
				...
				if (mult != 0) {
					for (int j = 0; j < nx; j++) {
						// B[i][j] -= B[k][j]*LU[i,k];
						B.setQuick(i, j, B.getQuick(i, j) - B.getQuick(k, j)
								* mult);
					}
				}
			}
	...
	public String toString() {
		...
		buf.append("\npivot = ");
		try {
			buf.append(String.valueOf(new cern.colt.list.IntArrayList(this
					.getPivot())));
		} catch (IllegalArgumentException exc) {
			buf.append(unknown + exc.getMessage());
		}
		...
		buf.append("\n\ninverse(A) = ");
		DoubleMatrix2D identity = cern.colt.matrix.DoubleFactory2D.dense
				.identity(LU.rows());
		try {
			this.solve(identity);
