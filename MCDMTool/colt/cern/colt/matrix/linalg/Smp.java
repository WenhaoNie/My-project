 ...
 */
class Smp {
	protected FJTaskRunnerGroup taskGroup; // a very efficient and light weight
											// thread pool
	...
	protected void run(final DoubleMatrix2D[] blocksA,
			final DoubleMatrix2D[] blocksB, final double[] results,
		...
		for (int i = 0; i < blocksA.length; i++) {
			...
			subTasks[i] = new FJTask() {
				public void run() {
					double result = function.apply(blocksA[k],
	...
	protected DoubleMatrix2D[] splitBlockedNN(DoubleMatrix2D A, int threshold,
		...
		int noOfTasks = (int) Math.min(flops / threshold, this.maxThreads); // each
																			// thread
																			// should
																			// process
																			// at
	...
	protected DoubleMatrix2D[][] splitBlockedNN(DoubleMatrix2D A,
			DoubleMatrix2D B, int threshold, long flops) {
	...
	protected DoubleMatrix2D[] splitStridedNN(DoubleMatrix2D A, int threshold,
			long flops) {
		/*
		 * determine how to split and parallelize best into blocks if more
		 * B.columns than tasks --> split B.columns, as follows:
		...
		for (int i = 0; i < noOfTasks; i++) {
			...
			if (!splitHoriz) {
				...
				blocks[i] = A.viewPart(0, i, A.rows(), A.columns() - i)
						.viewStrides(1, noOfTasks);
			} else {
				// split A along rows into blocks
				blocks[i] = A.viewPart(i, 0, A.rows() - i, A.columns())
						.viewStrides(noOfTasks, 1);
			}
		}
		return blocks;
	}
