 ...
 * <td>
 * <div align="center"><tt>0</tt></div></td>
 * <td>
 * <div align="center"><tt>0</tt></div></td>
 * <td>
 * <div align="center"><tt>1</tt></div></td>
 * <td><tt>3</tt></td>
 * <td align="center" valign="middle"><tt>0</tt></td>
 * <td align="center" valign="middle">
 * <div align="center"><tt>1</tt></div></td>
 * <td align="center" valign="middle">
 * <div align="center"><tt>2</tt></div></td>
 * </tr>
 * <tr align="center" valign="middle">
 ...
 * <td align="center" valign="middle">
 * <div align="center"><tt>3</tt></div></td>
 * <td align="center" valign="middle">
 * <div align="center"><tt>2</tt></div></td>
 * </tr>
 * <tr align="center" valign="middle">
 ...
 * <td align="center" valign="middle">
 * <div align="center"><tt>4</tt></div></td>
 * <td align="center" valign="middle">
 * <div align="center"><tt>3</tt></div></td>
 * </tr>
 * <tr align="center" valign="middle">
 ...
 * <td>
 * <div align="center"><tt>zero</tt></div></td>
 * <td>
 * <div align="center"><tt>diagonal</tt></div></td>
 * <td>
 * <div align="center"><tt>tridiagonal</tt></div></td>
 * <td><tt>upper triangular</tt></td>
 * <td align="center" valign="middle"><tt>lower triangular</tt></td>
 * <td align="center" valign="middle">
 * <div align="center"><tt>unstructured</tt></div></td>
 * <td align="center" valign="middle">
 * <div align="center"><tt>unstructured</tt></div></td>
 * </tr>
 * </table>
...
public class Property extends cern.colt.PersistentObject {
	...
	public void checkRectangular(DoubleMatrix2D A) {
		if (A.rows() < A.columns()) {
			throw new IllegalArgumentException("Matrix must be rectangular: "
					+ cern.colt.matrix.doublealgo.Formatter.shape(A));
		}
	}
	...
	public boolean equals(DoubleMatrix1D A, DoubleMatrix1D B) {
		...
		for (int i = size; --i >= 0;) {
			...
			// if (Math.abs(A.getQuick(i) - B.getQuick(i)) > epsilon) return
			// false;
			double x = A.getQuick(i);
	...
	public boolean equals(DoubleMatrix2D A, double value) {
		...
		for (int row = rows; --row >= 0;) {
			for (int column = columns; --column >= 0;) {
				...
				// if (Math.abs(value - A.getQuick(row,column)) > epsilon)
				// return false;
				double x = A.getQuick(row, column);
				double diff = Math.abs(value - x);
				if ((diff != diff)
						&& ((value != value && x != x) || value == x))
					diff = 0;
				if (!(diff <= epsilon))
					return false;
			}
	...
	public boolean equals(DoubleMatrix2D A, DoubleMatrix2D B) {
		...
		for (int row = rows; --row >= 0;) {
			for (int column = columns; --column >= 0;) {
				// if (!(A.getQuick(row,column) == B.getQuick(row,column)))
				// return false;
				// if (Math.abs((A.getQuick(row,column) -
				// B.getQuick(row,column)) > epsilon) return false;
				double x = A.getQuick(row, column);
	...
	public boolean equals(DoubleMatrix3D A, double value) {
		...
		for (int slice = A.slices(); --slice >= 0;) {
			for (int row = rows; --row >= 0;) {
				for (int column = columns; --column >= 0;) {
					// if (!(A.getQuick(slice,row,column) == value)) return
					// false;
					// if (Math.abs(value - A.getQuick(slice,row,column)) >
	...
	public boolean equals(DoubleMatrix3D A, DoubleMatrix3D B) {
		...
		for (int slice = slices; --slice >= 0;) {
			for (int row = rows; --row >= 0;) {
				for (int column = columns; --column >= 0;) {
					// if (!(A.getQuick(slice,row,column) ==
					// B.getQuick(slice,row,column))) return false;
					// if (Math.abs(A.getQuick(slice,row,column) -
					// B.getQuick(slice,row,column)) > epsilon) return false;
					double x = A.getQuick(slice, row, column);
 ...
 */
	...
	public boolean isDiagonal(DoubleMatrix2D A) {
		...
		for (int row = rows; --row >= 0;) {
			for (int column = columns; --column >= 0;) {
				...
				if (row != column
						&& !(Math.abs(A.getQuick(row, column)) <= epsilon))
					return false;
			}
		}
		return true;
	...
	public void setTolerance(double newTolerance) {
		if (this == DEFAULT || this == ZERO || this == TWELVE) {
			...
			// throw new
			// UnsupportedOperationException("Attempted to modify object."); //
			// since JDK1.2
		}
		tolerance = Math.abs(newTolerance);
	}
