...
public class Algebra extends cern.colt.PersistentObject {
	 ...
	 */
	public DoubleMatrix2D multOuter(DoubleMatrix1D x, DoubleMatrix1D y,
			DoubleMatrix2D A) {
		int rows = x.size();
		int columns = y.size();
		if (A == null)
	...
	public double norm1(DoubleMatrix1D x) {
		if (x.size() == 0)
			return 0;
		return x.aggregate(cern.jet.math.Functions.plus,
	...
	public double normInfinity(DoubleMatrix1D x) {
		...
		if (x.size() == 0)
			return 0;
		return x.aggregate(cern.jet.math.Functions.max,
				cern.jet.math.Functions.abs);
	...
	public DoubleMatrix1D permute(DoubleMatrix1D A, int[] indexes, double[] work) {
		 ...
		 * a >= size) throw new
		 * IndexOutOfBoundsException("invalid permutation"); if (i<0) return; //
		 * nothing to permute
		 */
	 ...
	 */
	public DoubleMatrix2D permute(DoubleMatrix2D A, int[] rowIndexes,
			int[] columnIndexes) {
	 ...
	 */
	public DoubleMatrix2D permuteColumns(DoubleMatrix2D A, int[] indexes,
			int[] work) {
	 ...
	 */
	public DoubleMatrix2D permuteRows(final DoubleMatrix2D A, int[] indexes,
			int[] work) {
		// check validity
		int size = A.rows();
		if (indexes.length != size)
	...
	public DoubleMatrix2D pow(DoubleMatrix2D A, int p) {
		...
		Blas blas = SmpBlas.smpBlas; // for parallel matrix mult; if not
										// initialized defaults to sequential
										// blas
		...
		if (p == 2) {
			blas.dgemm(false, false, 1, A, A, 0, T); // mult(A,A); // safes one
														// auxiliary matrix
														// allocation
			return T;
		}

		int k = cern.colt.bitvector.QuickBitVector.mostSignificantBit(p); // index
																			// of
																			// highest
																			// bit
																			// in
	 ...
	 * 
	 * @param the
	 *            Property object to be attached.
	 ...
	 *             .
	 */
	private DoubleMatrix2D subMatrix(DoubleMatrix2D A, int[] rowIndexes,
	 ...
	 *             .
	 */
	private DoubleMatrix2D subMatrix(DoubleMatrix2D A, int rowFrom, int rowTo,
			int[] columnIndexes) {
	 ...
	 */
	public DoubleMatrix2D subMatrix(DoubleMatrix2D A, int fromRow, int toRow,
			int fromColumn, int toColumn) {
		return A.viewPart(fromRow, fromColumn, toRow - fromRow + 1, toColumn
				- fromColumn + 1);
	}
	...
	public String toVerboseString(DoubleMatrix2D matrix) {
		/*
		 * StringBuffer buf = new StringBuffer(); String unknown =
		 * "Illegal operation or error: "; String constructionException =
		 * "Illegal operation or error upon construction: ";
		 * 
		 * buf.append(
		 * "------------------------------------------------------------------\n"
		 * ); buf.append(
		 * "LUDecomposition(A) --> isNonSingular, det, pivot, L, U, inverse(A)\n"
		 * ); buf.append(
		 * "------------------------------------------------------------------\n"
		 * );
		 */
		...
		try {
			...
			buf.append("\n\n" + constructionException
					+ " CholeskyDecomposition: " + exc.getMessage());
		}
		if (chol != null)
			buf.append("\n\n" + chol.toString());
		...
		try {
			...
			buf.append("\n\n" + constructionException
					+ " EigenvalueDecomposition: " + exc.getMessage());
		}
		if (eig != null)
			buf.append("\n\n" + eig.toString());
		...
		try {
			...
			buf.append("\n\n" + constructionException
					+ " SingularValueDecomposition: " + exc.getMessage());
		}
		if (svd != null)
			buf.append("\n\n" + svd.toString());
