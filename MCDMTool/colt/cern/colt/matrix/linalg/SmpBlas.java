 ...
 * multi processing boxes. Currently only a few algorithms are parallelised; the
 * others are fully functional, but run in sequential mode. Parallelised are:
 * <ul>
 * <li>{@link #dgemm dgemm} (matrix-matrix multiplication)</li>
 * <li>{@link #dgemv dgemv} (matrix-vector multiplication)</li>
 * <li>{@link #assign(DoubleMatrix2D,cern.colt.function.DoubleFunction)
 * assign(A,function)} (generalized matrix scaling/transform): Strong speedup
 * only for expensive functions like logarithm, sin, etc.</li>
 * <li>
 * {@link #assign(DoubleMatrix2D,DoubleMatrix2D,cern.colt.function.DoubleDoubleFunction)
 * assign(A,B,function)} (generalized matrix scaling/transform): Strong speedup
 * only for expensive functions like pow etc.</li>
 * </ul>
 * In all cases, no or only marginal speedup is seen for small problem sizes;
 * they are detected and the sequential algorithm is used.
 * 
 * <h4>Usage</h4> Call the static method {@link #allocateBlas} at the very
 * beginning of your program, supplying the main parameter for SmpBlas, the
 * number of available CPUs. The method sets the public global variable
 * <tt>SmpBlas.smpBlas</tt> to a blas using a maximum of <tt>CPUs</tt> threads,
 * each concurrently processing matrix blocks with the given sequential blas
 * algorithms. Normally there is no need to call <tt>allocateBlas</tt> more than
 * once. Then use <tt>SmpBlas.smpBlas.someRoutine(...)</tt> to run
 * <tt>someRoutine</tt> in parallel. E.g.
 * <table>
 * <td class="PRE">
 * 
 * <pre>
 ...
 * </td>
 * </table>
 * Even if you don't call a blas routine yourself, it often makes sense to
 * allocate a SmpBlas, because other matrix library routines sometimes call the
 * blas. So if you're lucky, you get parallel performance for free. <h4>Notes</h4>
 * <ul>
 ...
 * <li>Only improves performance on boxes with > 1 CPUs and VMs with <b>native
 * threads</b>.</li>
 * <li>Currently only improves performance when working on dense matrix types.
 * On sparse types, performance is likely to degrade (because of the
 * implementation of sub-range views)!</li>
 * <li>Implemented using Doug Lea's fast lightweight task framework (
 * {@link EDU.oswego.cs.dl.util.concurrent}) built upon Java threads, and geared
 * for parallel computation.</li>
 * </ul>
 * 
...
public class SmpBlas implements Blas {
	...
	protected Blas seqBlas; // blocks are operated on in parallel; for each
							// block this seq algo is used.
	...
	/**
	 * Constructs a blas using a maximum of
	 * <tt>maxThreads<tt> threads; each executing the given sequential algos.
	 */
	...
	/**
	 * Sets the public global variable <tt>SmpBlas.smpBlas</tt> to a blas using
	 * a maximum of <tt>maxThreads</tt> threads, each executing the given
	 * sequential algorithm; <tt>maxThreads</tt> is normally the number of CPUs.
	 * Call this method at the very beginning of your program. Normally there is
	 * no need to call this method more than once.
	 * 
	 * @param maxThreads
	 *            the maximum number of threads (= CPUs) to be used
	 * @param seqBlas
	 *            the sequential blas algorithms to be used on concurrently
	 *            processed matrix blocks.
	 */
	...
	public void assign(DoubleMatrix2D A, DoubleMatrix2D B,
			final cern.colt.function.DoubleDoubleFunction function) {
	...
	public void dgemm(final boolean transposeA, final boolean transposeB,
			final double alpha, final DoubleMatrix2D A, final DoubleMatrix2D B,
			final double beta, final DoubleMatrix2D C) {
		/*
		...
		if (B.rows() != n)
			throw new IllegalArgumentException(
					"Matrix2D inner dimensions must agree:" + A.toStringShort()
							+ ", " + B.toStringShort());
		if (C.rows() != m || C.columns() != p)
			throw new IllegalArgumentException("Incompatibel result matrix: "
					+ A.toStringShort() + ", " + B.toStringShort() + ", "
					+ C.toStringShort());
		if (A == C || B == C)
			throw new IllegalArgumentException("Matrices must not be identical");

		long flops = 2L * m * n * p;
		int noOfTasks = (int) Math.min(flops / 30000, this.maxThreads); // each
																		// thread
																		// should
																		// process
																		// at
		...
		if (noOfTasks < 2) { // parallelization doesn't pay off (too much start
								// up overhead)
		...
		for (int i = 0; i < noOfTasks; i++) {
			...
			subTasks[i] = new FJTask() {
				public void run() {
					seqBlas.dgemm(transposeA, transposeB, alpha, AA, BB, beta,
							CC);
	...
	public void dgemv(final boolean transposeA, final double alpha,
			DoubleMatrix2D A, final DoubleMatrix1D x, final double beta,
			DoubleMatrix1D y) {
		/*
		 * split A, as follows:
		 * 
		 * x x x x A xxx x y xxx x --- - xxx x xxx x --- - xxx x
		 */
		if (transposeA) {
	...
	public void dsymv(boolean isUpperTriangular, double alpha,
			DoubleMatrix2D A, DoubleMatrix1D x, double beta, DoubleMatrix1D y) {
	...
	protected double[] run(DoubleMatrix2D A, DoubleMatrix2D B,
			boolean collectResults, Matrix2DMatrix2DFunction fun) {
		DoubleMatrix2D[][] blocks;
		blocks = this.smp.splitBlockedNN(A, B, NN_THRESHOLD,
				A.rows() * A.columns());
		// blocks = this.smp.splitStridedNN(A, B, NN_THRESHOLD,
		// A.rows()*A.columns());
	...
	protected double[] run(DoubleMatrix2D A, boolean collectResults,
			Matrix2DMatrix2DFunction fun) {
		DoubleMatrix2D[] blocks;
		blocks = this.smp.splitBlockedNN(A, NN_THRESHOLD,
				A.rows() * A.columns());
		// blocks = this.smp.splitStridedNN(A, NN_THRESHOLD,
		// A.rows()*A.columns());
