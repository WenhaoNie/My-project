...
public abstract class ObjectMatrix3D extends AbstractMatrix3D {
	 ...
	 */
	public Object aggregate(cern.colt.function.ObjectObjectFunction aggr,
			cern.colt.function.ObjectFunction f) {
		if (size() == 0)
			return null;
	...
	public ObjectMatrix3D assign(ObjectMatrix3D y,
			cern.colt.function.ObjectObjectFunction function) {
		checkShape(y);
		for (int slice = slices; --slice >= 0;) {
			for (int row = rows; --row >= 0;) {
				for (int column = columns; --column >= 0;) {
	...
	public boolean equals(Object otherObj, boolean testForEquality) { // delta
		...
		if (!testForEquality) {
			for (int slice = slices; --slice >= 0;) {
				for (int row = rows; --row >= 0;) {
					for (int column = columns; --column >= 0;) {
						if (getQuick(slice, row, column) != other.getQuick(
								slice, row, column))
							return false;
					}
				}
			...
			for (int slice = slices; --slice >= 0;) {
				for (int row = rows; --row >= 0;) {
					for (int column = columns; --column >= 0;) {
						if (!(getQuick(slice, row, column) == null ? other
								.getQuick(slice, row, column) == null
								: getQuick(slice, row, column).equals(
										other.getQuick(slice, row, column))))
							return false;
					}
				}
	...
	public void getNonZeros(IntArrayList sliceList, IntArrayList rowList,
			IntArrayList columnList, ObjectArrayList valueList) {
		sliceList.clear();
		rowList.clear();
		columnList.clear();
		valueList.clear();
	 ...
	 */
	protected abstract ObjectMatrix2D like2D(int rows, int columns,
	 ...
	 * @return the new view.
	 */
	public ObjectMatrix3D viewPart(int slice, int row, int column, int depth,
			int height, int width) {
		return (ObjectMatrix3D) (view().vPart(slice, row, column, depth,
				height, width));
	}
	 ...
	 */
	public ObjectMatrix3D viewSelection(int[] sliceIndexes, int[] rowIndexes,
			int[] columnIndexes) {
		// check for "all"
	...
	public ObjectMatrix3D viewSorted(int row, int column) {
		return cern.colt.matrix.objectalgo.Sorting.mergeSort.sort(this, row,
				column);
	}
	 ...
	 *             .
	 */
	public ObjectMatrix3D viewStrides(int sliceStride, int rowStride,
			int columnStride) {
		return (ObjectMatrix3D) (view().vStrides(sliceStride, rowStride,
				columnStride));
	}
