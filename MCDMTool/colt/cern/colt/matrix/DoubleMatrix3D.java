...
public abstract class DoubleMatrix3D extends AbstractMatrix3D {
		...
		for (int slice = slices; --slice >= 0;) {
			for (int row = rows; --row >= 0;) {
				for (int column = columns - d; --column >= 0;) {
					a = aggr.apply(
							a,
							f.apply(getQuick(slice, row, column),
	...
	public DoubleMatrix3D assign(double[][][] values) {
		if (values.length != slices)
			throw new IllegalArgumentException(
					"Must have same number of slices: slices=" + values.length
							+ "slices()=" + slices());
		for (int slice = slices; --slice >= 0;) {
			...
			for (int row = rows; --row >= 0;) {
				for (int column = columns; --column >= 0;) {
					setQuick(slice, row, column,
							function.apply(getQuick(slice, row, column)));
				}
			}
		}
		return this;
	...
	public double get(int slice, int row, int column) {
		if (slice < 0 || slice >= slices || row < 0 || row >= rows
			...
			throw new IndexOutOfBoundsException("slice:" + slice + ", row:"
					+ row + ", column:" + column);
	 ...
	 */
	public void getNonZeros(IntArrayList sliceList, IntArrayList rowList,
			IntArrayList columnList, DoubleArrayList valueList) {
		sliceList.clear();
		rowList.clear();
		columnList.clear();
		valueList.clear();
	 ...
	 */
	protected abstract DoubleMatrix2D like2D(int rows, int columns,
	 ...
	 * @return the new view.
	 */
	public DoubleMatrix3D viewPart(int slice, int row, int column, int depth,
			int height, int width) {
		return (DoubleMatrix3D) (view().vPart(slice, row, column, depth,
				height, width));
	}
	 ...
	 */
	public DoubleMatrix3D viewSelection(int[] sliceIndexes, int[] rowIndexes,
			int[] columnIndexes) {
		// check for "all"
	 ...
	 * @return a new view.
	 */
	protected abstract DoubleMatrix3D viewSelectionLike(int[] sliceOffsets,
			int[] rowOffsets, int[] columnOffsets);
	...
	public DoubleMatrix3D viewSorted(int row, int column) {
		return cern.colt.matrix.doublealgo.Sorting.mergeSort.sort(this, row,
				column);
	}
	 ...
	 *             .
	 */
	public DoubleMatrix3D viewStrides(int sliceStride, int rowStride,
			int columnStride) {
		return (DoubleMatrix3D) (view().vStrides(sliceStride, rowStride,
				columnStride));
	}
	 ...
	 *         iterated over, <tt>true</tt> otherwise.
	 */
	private boolean xforEachCoordinate(
			final cern.colt.function.IntIntIntProcedure procedure) {
		for (int column = columns; --column >= 0;) {
			for (int slice = slices; --slice >= 0;) {
				for (int row = rows; --row >= 0;) {
					if (!procedure.apply(slice, row, column))
	 ...
	 * case of overlapping views, behaviour is unspecified. </pre>
	 * <p>
	 * <b>Example:</b>
	 * 
	 * <pre>
		...
		for (int k = 1; k < slices - 1; k++) {
			for (int i = 1; i < r; i++) {
				...
				for (int j = 1; j < c; j++) {
					...
					B.setQuick(k, i, j, function.apply(a000, a001, a002, a010,
							a011, a012, a020, a021, a022,

							a100, a101, a102, a110, a111, a112, a120, a121,
							a122,
	...
	public double zSum() {
		if (size() == 0)
			return 0;
		return aggregate(cern.jet.math.Functions.plus,
