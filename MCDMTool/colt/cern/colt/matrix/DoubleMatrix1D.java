...
public abstract class DoubleMatrix1D extends AbstractMatrix1D {
	...
	public DoubleMatrix1D assign(double[] values) {
		if (values.length != size)
			throw new IllegalArgumentException(
					"Must have same number of cells: length=" + values.length
							+ "size()=" + size());
		for (int i = size; --i >= 0;) {
			setQuick(i, values[i]);
		}
	...
	public DoubleMatrix1D assign(DoubleMatrix1D y,
		...
		if (function == cern.jet.math.Functions.mult) { // x[i] = x[i] * y[i]
			...
			for (int index = nonZeroIndexes.size(); --index >= 0;) {
				...
				setQuick(i, getQuick(i) * y.getQuick(i)); // x[i] * y[i] for all
															// nonZeros
				j++;
			}
	...
	public DoubleMatrix1D viewSelection(
			cern.colt.function.DoubleProcedure condition) {
	...
	public double zDotProduct(DoubleMatrix1D y, int from, int length,
		...
		int i;
		double sum = 0;
		while ((--length >= 0) && (index < s)
				&& ((i = nonZeroIndexElements[index]) < tail)) {
			sum += getQuick(i) * y.getQuick(i);
			index++;
		}
	...
	protected double zDotProduct(DoubleMatrix1D y, IntArrayList nonZeroIndexes) {
		...
		/*
		 * double sum = 0; int[] nonZeroIndexElements =
		 * nonZeroIndexes.elements(); for (int index=nonZeroIndexes.size();
		 * --index >= 0; ) { int i = nonZeroIndexElements[index]; sum +=
		 * getQuick(i) * y.getQuick(i); } return sum;
		 */
	}
