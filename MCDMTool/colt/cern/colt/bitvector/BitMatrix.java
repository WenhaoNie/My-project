...
public class BitMatrix extends cern.colt.PersistentObject {
	...
	protected void checkDimensionCompatibility(BitMatrix other) {
		if (columns != other.columns() || rows != other.rows())
			throw new IllegalArgumentException(
					"Incompatible dimensions: (columns,rows)=(" + columns + ","
							+ rows + "), (other.columns,other.rows)=("
	...
	protected void containsBox(int column, int row, int width, int height) {
		if (column < 0 || column + width > columns || row < 0
				|| row + height > rows)
			throw new IndexOutOfBoundsException("column:" + column + ", row:"
	...
	protected void elements(long[] bits, int columns, int rows) {
		if (columns < 0 || columns < 0
				|| columns * rows > bits.length * QuickBitVector.BITS_PER_UNIT)
			throw new IllegalArgumentException();
		this.bits = bits;
	...
	public boolean forEachCoordinateInState(boolean state,
			final cern.colt.function.IntIntProcedure procedure) {
		/*
	 ...
	 */
	public void replaceBoxWith(int column, int row, int width, int height,
			BitMatrix source, int sourceColumn, int sourceRow) {
		...
		if (source == this) {
			...
			Rectangle sourceRect = new Rectangle(sourceColumn, sourceRow,
					width, height);
		...
		for (; --height >= 0; row++, sourceRow++) {
			...
			destVector.replaceFromToWith(offset, offset + width - 1,
					sourceVector, sourceOffset);
		}
	}
