...
public class BitVector extends cern.colt.PersistentObject {
	...
	protected void checkSize(BitVector other) {
		if (nbits > other.size())
			throw new IllegalArgumentException("Incompatible sizes: size="
					+ nbits + ", other.size()=" + other.size());
	}
	...
	public boolean forEachIndexFromToInState(int from, int to, boolean state,
			cern.colt.function.IntProcedure procedure) {
		/*
	 ...
	 *             .
	 */
	public void replaceFromToWith(int from, int to, BitVector source,
			int sourceFrom) {
		if (nbits == 0 || to == from - 1)
			return;
		checkRangeFromTo(from, to, nbits);
		 ...
		 * QuickBitVector
		 * .put(theBits,i,QuickBitVector.get(sourceBits,sourceFrom)); }
		 */
		...
		for (int i = blocks; --i >= 0;) {
			long val = QuickBitVector.getLongFromTo(sourceBits, sourceFrom,
					sourceFrom + bitsPerUnitMinusOne);
			QuickBitVector.putLongFromTo(theBits, val, from, from
					+ bitsPerUnitMinusOne);
	...
	public void replaceFromToWith(int from, int to, boolean value) {
		...
		if (fromUnit == toUnit) { // only one unit to do
			QuickBitVector.putLongFromTo(theBits, filler, bitIndex, bitIndex
					+ to - from);
	...
	public void setSize(int newSize) {
		if (newSize != size()) {
			...
			newVector.replaceFromToWith(0, Math.min(size(), newSize) - 1, this,
					0);
