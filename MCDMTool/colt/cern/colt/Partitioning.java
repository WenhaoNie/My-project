...
public class Partitioning extends Object {
	 ...
	 *         if the key is found.
	 */
	private static int binarySearchFromTo(int a, int from, int to,
			IntComparator comp) {
		while (from <= to) {
			int mid = (from + to) / 2;
	 ...
	 */
	public static void dualPartition(double[] list, double[] secondary,
			int from, int to, double[] splitters, int splitFrom, int splitTo,
			int[] splitIndexes) {
		...
		if (splitFrom == splitTo) { // we don't really have a choice
			...
			medianIndex = Sorting.binarySearchFromTo(splitters, list[m],
					splitFrom, splitTo);
			if (medianIndex < 0)
		...
		if (splitFrom <= medianIndex - 1) {
			dualPartition(list, secondary, from, splitIndex, splitters,
					splitFrom, medianIndex - 1, splitIndexes);
		}

		// recursively partition right half
		if (medianIndex + 1 <= splitTo) {
			dualPartition(list, secondary, splitIndex + 1, to, splitters,
	 ...
	 * <tt>int[]</tt> arrays.
	 */
	public static int dualPartition(double[] list, double[] secondary,
			int from, int to, double splitter) {
	 ...
	 */
	public static void dualPartition(int[] list, int[] secondary, int from,
			int to, int[] splitters, int splitFrom, int splitTo,
			int[] splitIndexes) {
	 ...
	 * Same as for single-partition methods.
	 */
	public static int dualPartition(int[] list, int[] secondary, int from,
			int to, int splitter) {
	 ...
	 */
	public static void genericPartition(int from, int to, int splitFrom,
			int splitTo, int[] splitIndexes, IntComparator comp,
			IntComparator comp2, IntComparator comp3, Swapper swapper) {
		int splitter; // int, double --> template type dependent
		...
		// recursively partition left half
		if (splitFrom <= medianIndex - 1) {
			genericPartition(from, splitIndex, splitFrom, medianIndex - 1,
					splitIndexes, comp, comp2, comp3, swapper);
		}

		// recursively partition right half
		if (medianIndex + 1 <= splitTo) {
			genericPartition(splitIndex + 1, to, medianIndex + 1, splitTo,
					splitIndexes, comp, comp2, comp3, swapper);
		}
	}
	 ...
	 */
	private static int genericPartition(int from, int to, int splitter,
			IntComparator comp, Swapper swapper) {
		for (int i = from - 1; ++i <= to;) {
	 ...
	 */
	public static void partition(double[] list, int from, int to,
		...
		// recursively partition left half
		if (splitFrom <= medianIndex - 1) {
			partition(list, from, splitIndex, splitters, splitFrom,
					medianIndex - 1, splitIndexes);
		}

		// recursively partition right half
		if (medianIndex + 1 <= splitTo) {
			partition(list, splitIndex + 1, to, splitters, medianIndex + 1,
					splitTo, splitIndexes);
		}
	}
	 ...
	 */
	public static void partition(int[] list, int from, int to, int[] splitters,
		 ...
		 * // swap all elements < splitter to front element = list[i]; if
		 * (element < splitter) { list[i] = list[head]; list[head++] = element;
	...
	public static int partition(int[] list, int from, int to, int splitter) {
		...
		/*
		 * System.out.println(); if (from<=to) {
		 * System.out.println("SORT WORKING: from="
		 * +from+", to="+to+", splitter="+splitter); } else {
		...
		/*
		 * for (int i=from-1; ++i<=to; ) { if (list[i] < splitter) { int element
		 * = list[i]; list[i] = list[from]; list[from++] = element; } }
		 */
	 ...
	 */
	public static void partition(Object[] list, int from, int to,
		...
		if (splitIndex < from) { // no element falls into this bin
			...
			int i = medianIndex - 1;
			while (i >= splitFrom
					&& (!(comp.compare(splitter, splitters[i]) < 0)))
				splitIndexes[i--] = splitIndex;
			splitFrom = medianIndex + 1;
		} else if (splitIndex >= to) { // all elements fall into this bin
			// all bins with splitters[i] >= splitter are empty
		...
		if (splitFrom <= medianIndex - 1) {
			partition(list, from, splitIndex, splitters, splitFrom,
					medianIndex - 1, splitIndexes, comp);
		}

		// recursively partition right half
		if (medianIndex + 1 <= splitTo) {
			partition(list, splitIndex + 1, to, splitters, medianIndex + 1,
					splitTo, splitIndexes, comp);
		}
	}
	 ...
	 */
	public static int partition(Object[] list, int from, int to,
			Object splitter, java.util.Comparator comp) {
	 ...
	 * .
	 */
	public static void partition(DoubleArrayList list, int from, int to,
			DoubleArrayList splitters, IntArrayList splitIndexes) {
		partition(list.elements(), from, to, splitters.elements(), 0,
				splitters.size() - 1, splitIndexes.elements());
	}
	 ...
	 * .
	 */
	public static void partition(IntArrayList list, int from, int to,
			IntArrayList splitters, IntArrayList splitIndexes) {
		partition(list.elements(), from, to, splitters.elements(), 0,
				splitters.size() - 1, splitIndexes.elements());
	}
	 ...
	 * than <tt>int[]</tt> arrays.
	 */
	public static void triplePartition(double[] list, double[] secondary,
			double[] tertiary, int from, int to, double[] splitters,
			int splitFrom, int splitTo, int[] splitIndexes) {
		double splitter; // int, double --> template type dependent
		...
		// Establish invariant: list[i] < splitter <= list[j] for
		// i=from..medianIndex and j=medianIndex+1 .. to
		int splitIndex = triplePartition(list, secondary, tertiary, from, to,
				splitter);
		splitIndexes[medianIndex] = splitIndex;
		...
		// recursively partition left half
		if (splitFrom <= medianIndex - 1) {
			triplePartition(list, secondary, tertiary, from, splitIndex,
					splitters, splitFrom, medianIndex - 1, splitIndexes);
		}

		// recursively partition right half
		if (medianIndex + 1 <= splitTo) {
			triplePartition(list, secondary, tertiary, splitIndex + 1, to,
					splitters, medianIndex + 1, splitTo, splitIndexes);
		}
	}
	 ...
	 * <tt>int[]</tt> arrays.
	 */
	public static int triplePartition(double[] list, double[] secondary,
			double[] tertiary, int from, int to, double splitter) {
		double element; // int, double --> template type dependent
		for (int i = from - 1; ++i <= to;) {
			element = list[i];
			if (element < splitter) {
	 ...
	 * Same as for single-partition methods.
	 */
	public static void triplePartition(int[] list, int[] secondary,
			int[] tertiary, int from, int to, int[] splitters, int splitFrom,
	 ...
	 * Same as for single-partition methods.
	 */
	public static int triplePartition(int[] list, int[] secondary,
			int[] tertiary, int from, int to, int splitter) {
		int element; // int, double --> template type dependent
		for (int i = from - 1; ++i <= to;) {
			element = list[i];
			if (element < splitter) {
