...
public class BreitWignerMeanSquare extends BreitWigner {
	...
	protected static BreitWigner shared = new BreitWignerMeanSquare(1.0, 0.2,
			1.0, makeDefaultGenerator());
	 ...
	 */
	public BreitWignerMeanSquare(double mean, double gamma, double cut,
	...
	public double nextDouble(double mean, double gamma, double cut) {
		...
		if (cut == Double.NEGATIVE_INFINITY) { // don't cut
			...
			double lower = Math
					.atan((tmp * tmp - mean * mean) / (mean * gamma));
			double upper = Math
					.atan(((mean + cut) * (mean + cut) - mean * mean)
							/ (mean * gamma));
