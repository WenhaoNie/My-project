...
public class RandomSampler extends cern.colt.PersistentObject {
	...
	public RandomSampler(long n, long N, long low, RandomEngine randomGenerator) {
		...
		if (randomGenerator == null)
			randomGenerator = cern.jet.random.AbstractDistribution
	...
	public Object clone() {
		...
		copy.my_RandomGenerator = (RandomEngine) this.my_RandomGenerator
				.clone();
		return copy;
	}
	 ...
	 */
	protected static void rejectMethodD(long n, long N, int count, long low,
			long[] values, int fromIndex, RandomEngine randomGenerator) {
		/*
		...
		while (n > 1 && count > 0) { // && threshold<N) {
			...
			for (;;) {
				...
				if (Nreal / (-X + Nreal) >= y1
						* Math.exp(Math.log(y2) * nmin1inv)) {
					// accept !
					Vprime = Math.exp(Math.log(randomGenerator.raw())
							* nmin1inv);
					break; // break inner loop
				}
				Vprime = Math.exp(Math.log(randomGenerator.raw()) * ninv);
	 ...
	 *            to use the default random number generator.
	 */
	public static void sample(long n, long N, int count, long low,
			long[] values, int fromIndex, RandomEngine randomGenerator) {
		if (n <= 0 || count <= 0)
			return;
		if (count > n)
	 ...
	 *            a random number generator.
	 */
	protected static void sampleMethodA(long n, long N, int count, long low,
			long[] values, int fromIndex, RandomEngine randomGenerator) {
	 ...
	 *            a random number generator.
	 */
	protected static void sampleMethodD(long n, long N, int count, long low,
			long[] values, int fromIndex, RandomEngine randomGenerator) {
		double nreal, Nreal, ninv, nmin1inv, U, X, Vprime, y1, y2, top, bottom, negSreal, qu1real;
	...
	protected static void testNegAlphaInv(String args[]) {
		 ...
		 * 
		 * long[] alphas = {-104, -52, -26, -13, -8, -4, -2}; for (int i=0;
		 * i<alphas.length; i++) { negalphainv = alphas[i];
