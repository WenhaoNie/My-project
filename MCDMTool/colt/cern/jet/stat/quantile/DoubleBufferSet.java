...
class DoubleBufferSet extends BufferSet {
	...
	public DoubleBuffer collapse(DoubleBuffer[] buffers) {
		...
		// sequence
		double[] outputValues = this.getValuesAtPositions(buffers,
				triggerPositions);
	 ...
	 */
	protected double[] getValuesAtPositions(DoubleBuffer[] buffers,
	...
	public double phi(double element) {
		...
		for (int i = buffers.length; --i >= 0;) {
			if (!buffers[i].isEmpty()) {
				elementsLessThanOrEqualToElement += buffers[i].weight
						* buffers[i].rank(element);
			}
		}
