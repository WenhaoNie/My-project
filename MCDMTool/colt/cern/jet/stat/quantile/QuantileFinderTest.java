...
class QuantileFinderTest {
	...
	public static double epsilon(int size, double phi, double rank) {
		double s = size;
		// System.out.println("\n");
		// System.out.println("s="+size+", rank="+rank+", phi="+phi+", eps="+Math.abs((rank)/s
		// - phi));
		// System.out.println("\n");
	 ...
	 * Observed epsilon
	 */
	public static double epsilon(DoubleArrayList sortedList, double phi,
			double element) {
		double rank = cern.jet.stat.Descriptive.rankInterpolated(sortedList,
				element);
	...
	public static double epsilon(DoubleArrayList sortedList,
			DoubleQuantileFinder finder, double phi) {
		double element = finder.quantileElements(
				new DoubleArrayList(new double[] { phi })).get(0);
	...
	public static double observedEpsilonAtPhi(double phi,
		...
		exactFinder.quantileElements(new DoubleArrayList(new double[] { phi }))
				.get(0); // just to ensure exactFinder is sorted
		double approxElement = approxFinder.quantileElements(
				new DoubleArrayList(new double[] { phi })).get(0);
		// System.out.println("approxElem="+approxElement);
		IntArrayList approxRanks = binaryMultiSearch(exactFinder.buffer,
				approxElement);
	 ...
	 *            double[]
	 */
	public static DoubleArrayList observedEpsilonsAtPhis(DoubleArrayList phis,
			ExactDoubleQuantileFinder exactFinder,
			DoubleQuantileFinder approxFinder, double desiredEpsilon) {
		DoubleArrayList epsilons = new DoubleArrayList(phis.size());

		for (int i = phis.size(); --i >= 0;) {
			double epsilon = observedEpsilonAtPhi(phis.get(i), exactFinder,
					approxFinder);
			epsilons.add(epsilon);
			if (epsilon > desiredEpsilon)
				System.out.println("Real epsilon = " + epsilon
				...
				for (int e = 0; e < epsilons.length; e++) {
					...
					for (int d = 0; d < deltas.length; d++) {
						...
						for (int knownCounter = 0; knownCounter < 2; knownCounter++) {
							...
							DoubleQuantileFinder finder = QuantileFinderFactory
									.newDoubleQuantileFinder(known_N, N,
											epsilon, delta, p, null);
							 ...
							 * result =
							 * QuantileFinderFactory.unknown_N_compute_B_and_K
							 * (epsilon, delta, p); long b1 = result[0]; long k1
							 * = result[1];
							 * 
							...
							System.out.print("         (known, d)=(" + knownStr
									+ ", " + delta + ") --> ");
							...
							System.out.print(")=(" + mem * 8.0 / 1024.0
									/ 1024.0 + ",  " + mem / 1000.0 + ",  "
	...
	public static void testLocalVarDeclarationSpeed(int size) {
		 ...
		 * (int j=0; j<size; j++) { DoubleBuffer buffer=null; int val=10; double
		 * f=1.0f; } } System.out.println(timer.stop());
		 */
 ...
 */
	public static void testQuantileCalculation(String[] args) {
		...
		approxFinder = QuantileFinderFactory.newDoubleQuantileFinder(false,
				max_N, epsilon, delta, quantiles, null);
		...
		DoubleQuantileFinder exactFinder = QuantileFinderFactory
				.newDoubleQuantileFinder(false, -1, 0.0, delta, quantiles, null);
		...
		// approxFinder.close();
		DoubleArrayList approxQuantiles = approxFinder
				.quantileElements(new DoubleArrayList(phis));
		...
		if (computeExactQuantilesAlso) {
			...
			// exactFinder.close();
			DoubleArrayList exactQuantiles = exactFinder
					.quantileElements(new DoubleArrayList(phis));
			timer.stop().display();
			...
			DoubleArrayList observedEpsilons = observedEpsilonsAtPhis(
					new DoubleArrayList(phis),
					(ExactDoubleQuantileFinder) exactFinder, approxFinder,
					epsilon);
			System.out.println("observedEpsilons=" + observedEpsilons);

			double element = 1000.0f;

			System.out.println("exact phi(" + element + ")="
					+ exactFinder.phi(element));
			System.out.println("apprx phi(" + element + ")="
					+ approxFinder.phi(element));

			System.out.println("exact elem(phi("
					+ element
					+ "))="
					+ exactFinder.quantileElements(new DoubleArrayList(
							new double[] { exactFinder.phi(element) })));
			System.out.println("apprx elem(phi("
					+ element
					+ "))="
					+ approxFinder.quantileElements(new DoubleArrayList(
							new double[] { approxFinder.phi(element) })));
		}
	}
	...
	public static void testRank() {
		DoubleArrayList list = new DoubleArrayList(new double[] { 1.0f, 5.0f,
				5.0f, 5.0f, 7.0f, 10.f });
