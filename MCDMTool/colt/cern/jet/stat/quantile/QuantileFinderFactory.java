 ...
 * 
 * <p>
 * Use methods <tt>newXXX(...)</tt> to get new instances of one of the following
 * quantile finders.
 * 
 * <p>
 ...
 * <p>
 * <p>
 * <b>3. Approximate quantile finding algorithm for unknown <tt>N</tt> requiring
 * only one pass and little main memory.</b>
 * </p>
...
public class QuantileFinderFactory extends Object {
	 ...
	 */
	public static long[] known_N_compute_B_and_K(long N, double epsilon,
			double delta, int quantiles, double[] returnSamplingRate) {
		...
		for (int b = 2; b <= maxBuffers; b++) {
			...
			while (h <= maxHeight && // skip heights until x<=0
					(h - 2) * (Arithmetic.binomial(b + h - 2, h - 1))
							...
							+ (Arithmetic.binomial(b + h - 3, h - 2)) - c > 0.0) {
				h++;
			}
			...
			for (long h = 3; h < maxHeight; h++) {
				...
				if (delta > 0.0) {
					...
					if (b * k < memory) {
						...
						ret_b = b;
						memory = b * k;
						sampling_rate = N_double * 2.0 * epsilon * epsilon
								/ logarithm;
					}
				}
			}
	...
	public static DoubleQuantileFinder newDoubleQuantileFinder(boolean known_N,
			long N, double epsilon, double delta, int quantiles,
		...
		if (known_N) {
			...
			if (true) { // no maximum N provided by user.
				if (b1 == 1)
					return new ExactDoubleQuantileFinder();
				return new UnknownDoubleQuantileEstimator((int) b1, (int) k1,
						(int) h1, preComputeEpsilon, generator);
			}
			...
			long[] resultKnown = known_N_compute_B_and_K(N, epsilon, 0,
					quantiles, samplingRate);
			...
			if (b2 * k2 < b1 * k1) { // the KnownFinder is smaller
				if (b2 == 1)
					return new ExactDoubleQuantileFinder();
				return new KnownDoubleQuantileEstimator((int) b2, (int) k2, N,
						samplingRate[0], generator);
			}
	...
	public static cern.colt.list.DoubleArrayList newEquiDepthPhis(int quantiles) {
		cern.colt.list.DoubleArrayList phis = new cern.colt.list.DoubleArrayList(
				quantiles - 1);
	...
	public static long[] unknown_N_compute_B_and_K(double epsilon,
		 ...
		 * (epsilon/2.0,delta,quantilesToPrecompute);
		 * 
	...
	protected static long[] unknown_N_compute_B_and_K_raw(double epsilon,
		...
		double logDelta = Math.log(2.0 / (delta / quantiles))
				/ (2.0 * epsilon * epsilon);
