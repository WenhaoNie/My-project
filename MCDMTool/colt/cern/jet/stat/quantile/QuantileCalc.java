...
class QuantileCalc extends Object {
	...
	protected static long[] known_N_compute_B_and_K_quick(long N, double epsilon) {
		...
		for (int b = 2; b <= maxBuffers; b++) {
			...
			if (h >= maxHeight
					&& (h - 2)
							* ((double) Math.round(binomial(b + h - 2, h - 1)))
							- ((double) Math.round(binomial(b + h - 3, h - 3)))
							+ ((double) Math.round(binomial(b + h - 3, h - 2)))
							- c > 0.0) {
	...
	protected static long[] known_N_compute_B_and_K_slow(long N,
		...
		for (long b = 2; b < maxBuffers; b++)
			for (long h = 3; h < maxHeight; h++) {
				...
				if ((b * tmp < memory)
						&& ((h - 2) * binomial - binomial(b + h - 3, h - 3)
								+ binomial(b + h - 3, h - 2) <= c)) {
					ret_k = tmp;
					ret_b = b;
					memory = ret_k * b;
					sampling_rate = 1.0;
				...
				if (delta > 0.0) {
					double t = (h - 2) * binomial(b + h - 2, h - 1)
							- binomial(b + h - 3, h - 3)
							+ binomial(b + h - 3, h - 2);
			...
			for (int s = 0; s < sizes.length; s++) {
				...
				for (int d = 0; d < deltas.length; d++) {
					...
					for (int e = 0; e < epsilons.length; e++) {
						...
						long[] result;
						if (known_N) {
							result = known_N_compute_B_and_K(N, epsilon, delta,
									p, returnSamplingRate);
						} else {
							result = unknown_N_compute_B_and_K(epsilon, delta,
									p);
						}

						long b = result[0];
						long k = result[1];
						System.out.print("         (e,d,N,p)=(" + epsilon + ","
								+ delta + "," + N + "," + p + ") --> ");
			...
			for (int b = 2; b <= max_b; b++) {
				for (int h = 2; h <= max_h; h++) {
					...
					if (alpha_one_OK || alpha_two_OK) {
						...
						// (h+1)/(2*epsilon)))
						long k = ceiling(Math.max(d / alpha, (h + 1)
								/ (2.0 * epsilon)));
