...
class UnknownDoubleQuantileEstimator extends DoubleQuantileEstimator {
	 ...
	 */
	public UnknownDoubleQuantileEstimator(int b, int k, int h,
			double precomputeEpsilon, RandomEngine generator) {
	 ...
	 */
	public Object clone() {
		UnknownDoubleQuantileEstimator copy = (UnknownDoubleQuantileEstimator) super
				.clone();
	...
	public DoubleArrayList quantileElements(DoubleArrayList phis) {
		...
		int quantilesToPrecompute = (int) Utils
				.epsilonCeiling(1.0 / precomputeEpsilon);
		/*
	...
	protected static void sortAscendingByLevel(DoubleBuffer[] fullBuffers) {
		new ObjectArrayList(fullBuffers).quickSortFromTo(0,
				fullBuffers.length - 1, new java.util.Comparator() {
	...
	public String toString() {
		...
		return buf + ", h=" + currentTreeHeight + ", hStartSampling="
				+ treeHeightStartingSampling + ", precomputeEpsilon="
				+ precomputeEpsilon + ")";
	}
}
