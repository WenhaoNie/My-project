...
abstract class DoubleQuantileEstimator extends cern.colt.PersistentObject
	...
	public void addAllOfFromTo(DoubleArrayList values, int from, int to) {
		 ...
		 * values.elements(); int theSize=values.size(); for (int i=0;
		 * i<theSize; ) add(theValues[i++]);
		 */
	...
	public Object clone() {
		...
		if (this.bufferSet != null) {
			...
			if (this.currentBufferToFill != null) {
				int index = new ObjectArrayList(this.bufferSet.buffers)
						.indexOf(this.currentBufferToFill, true);
	...
	public DoubleArrayList quantileElements(DoubleArrayList phis) {
		...
		// sequence
		return new DoubleArrayList(bufferSet.getValuesAtPositions(fullBuffers,
				triggerPositions));
	}
	...
	public String toString() {
		...
		int b = bufferSet.b();
		int k = bufferSet.k();
		return s + "(mem=" + memory() + ", b=" + b + ", k=" + k + ", size="
				+ size() + ", totalSize=" + this.bufferSet.totalSize() + ")";
	}
