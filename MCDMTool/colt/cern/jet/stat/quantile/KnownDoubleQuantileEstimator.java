...
class KnownDoubleQuantileEstimator extends DoubleQuantileEstimator {
	 ...
	 */
	public KnownDoubleQuantileEstimator(int b, int k, long N,
			double samplingRate, RandomEngine generator) {
		...
		if (this.samplingRate <= 1.0) {
			...
			this.samplingAssistant = new RandomSamplingAssistant(
	...
	public void clear() {
		...
		if (assist != null) {
			this.samplingAssistant = new RandomSamplingAssistant(
					Arithmetic.floor(N / samplingRate), N,
	 ...
	 */
	public Object clone() {
		KnownDoubleQuantileEstimator copy = (KnownDoubleQuantileEstimator) super
				.clone();
		if (this.samplingAssistant != null)
			copy.samplingAssistant = (RandomSamplingAssistant) copy.samplingAssistant
					.clone();
		return copy;
	}
 ...
 */
	...
	public DoubleArrayList quantileElements(DoubleArrayList phis) {
		...
		if (partial != null) { // any auxiliary infinities needed?
			...
			this.beta = (this.totalElementsFilled + missingValues)
					/ (double) this.totalElementsFilled;
		} else {
			this.beta = 1.0;
		}
