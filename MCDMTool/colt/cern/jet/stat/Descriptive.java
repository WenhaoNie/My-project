...
public class Descriptive extends Object {
	 ...
	 * faster
	 */
	private static double covariance2(DoubleArrayList data1,
	 ...
	 */
	public static void frequencies(DoubleArrayList sortedData,
			DoubleArrayList distinctValues, IntArrayList frequencies) {
	...
	public static double geometricMean(DoubleArrayList data) {
		return geometricMean(data.size(),
	 ...
	 * already recorded elements. This is interesting for interactive online
	 ...
	 * <tt>b(k,i) = </tt>{@link cern.jet.math.Arithmetic#binomial(long,long)
	 * binomial(k,i)} and <br>
	...
	public static void incrementalUpdateSumsOfPowers(DoubleArrayList data,
		...
		if (fromSumIndex == toSumIndex
				|| (fromSumIndex >= -1 && toSumIndex <= 5)) { // handle quicker
			for (int i = fromSumIndex; i <= toSumIndex; i++) {
				sumOfPowers[i - fromSumIndex] += sumOfPowerDeviations(data, i,
						0.0, from, to);
			}
			return;
		}
	...
	public static void incrementalWeightedUpdate(DoubleArrayList data,
		...
		if (dataSize != weights.size())
			throw new IllegalArgumentException("from=" + from + ", to=" + to
					+ ", data.size()=" + dataSize + ", weights.size()="
					+ weights.size());
	...
	public static double moment(int k, double c, int size, double[] sumOfPowers) {
		...
		for (int i = 0; i <= k; i++) {
			...
			if (i == 0)
				...
				y = Math.pow(c, i);
			// sum += sign *
			sum += sign * cern.jet.math.Arithmetic.binomial(k, i) * y
					* sumOfPowers[k - i];
			sign = -sign;
		}
		/*
	 ...
	 */
	public static double pooledMean(int size1, double mean1, int size2,
			double mean2) {
	 ...
	 */
	public static double pooledVariance(int size1, double variance1, int size2,
			double variance2) {
	 ...
	 */
	public static double sampleKurtosis(int size, double moment4,
			double sampleVariance) {
		int n = size;
		double s2 = sampleVariance; // (y-ymean)^2/(n-1)
		double m4 = moment4 * n; // (y-ymean)^4
		return m4 * n * (n + 1) / ((n - 1) * (n - 2) * (n - 3) * s2 * s2) - 3.0
				* (n - 1) * (n - 1) / ((n - 2) * (n - 3));
	}
	...
	public static double sampleKurtosis(DoubleArrayList data, double mean,
			double sampleVariance) {
		return sampleKurtosis(data.size(), moment(data, 4, mean),
				sampleVariance);
	}
	...
	public static double sampleKurtosisStandardError(int size) {
		int n = size;
		return Math.sqrt(24.0 * n * (n - 1) * (n - 1)
				/ ((n - 3) * (n - 2) * (n + 3) * (n + 5)));
	}
	 ...
	 *            the <b>sample variance</b>.
	 */
	public static double sampleSkew(int size, double moment3,
			double sampleVariance) {
		int n = size;
	...
	public static double sampleStandardDeviation(int size, double sampleVariance) {
		...
		if (n > 30) {
			...
			Cn = Math.sqrt((n - 1) * 0.5) * Gamma.gamma((n - 1) * 0.5)
					/ Gamma.gamma(n * 0.5);
		}
		return Cn * s;
	}
	 ...
	 */
	public static double sampleVariance(int size, double sum,
			double sumOfSquares) {
	...
	public static double sampleWeightedVariance(double sumOfWeights,
			double sumOfProducts, double sumOfSquaredProducts) {
		return (sumOfSquaredProducts - sumOfProducts * sumOfProducts
				/ sumOfWeights)
				/ (sumOfWeights - 1);
	}
	 ...
	 *         sorted ascending.
	 */
	public static DoubleArrayList[] split(DoubleArrayList sortedList,
		...
		while (nextStart < listSize && i < noOfBins - 1) {
			...
			if (index < 0) { // splitValue not found
				...
				bins[i].addAllOfFromTo(sortedList, nextStart,
						insertionPosition - 1);
				nextStart = insertionPosition;
			} else { // splitValue found
						// For multiple identical elements ("runs"),
						// binarySearch does not define which of all valid
						// indexes is returned.
		...
		bins[noOfBins - 1].addAllOfFromTo(sortedList, nextStart,
				sortedList.size() - 1);
