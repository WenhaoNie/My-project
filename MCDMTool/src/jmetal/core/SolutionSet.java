...
public class SolutionSet implements Serializable {
  ...
  */
  ...
  /**
   * Writes the objective function values, decision variables and ranks
   * by Wenhao
   */
  public void printToFile(String path){
	  try {
		  ...
		  for (int i = 0; i < solutionsList_.size(); i++) {
		      ...
		      bw.write(solutionsList_.get(i).getRank()+","+solutionsList_.get(i).toString()+",");
		      for (int j = 0; j < numberOfVariables; j++){
		    	  bw.write(solutionsList_.get(i).getDecisionVariables()[j].toString() + ",");
		      }
		      bw.newLine();
  ...
  /**
   * Writes the objective function values, decision variables and ranks
   * Also for individual products, writes time to market and success count
   * by Wenhao
   */
  public void printSuccessToFile(String path){
	  try {
		  /* Open the file */
		  FileOutputStream fos   = new FileOutputStream(path)     ;
		  OutputStreamWriter osw = new OutputStreamWriter(fos)    ;
  ...
  /**
   * Writes the runMean and runSTD to files
   * by Wenhao
   */
  public void printRunningAverage(String path){
	  try {
		  /* Open the file */
		  FileOutputStream fos   = new FileOutputStream(path)     ;
		  OutputStreamWriter osw = new OutputStreamWriter(fos)    ;
		  ...
		  for (int i = 0; i < solutionsList_.size(); i++) {
		      ...
		      bw.write(i+","+solutionsList_.get(i).converge+",");
		      for(int j=0;j<solutionsList_.get(i).runMean.length;j++){
		    	  bw.write(solutionsList_.get(i).runMean[j]+",");
		      }
		      bw.newLine();
